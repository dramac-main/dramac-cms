# DRAMAC CMS — INVFIX Session 11 Prompt
...
Do **not** assume Session 10 closed INVFIX-08.

The current repo still tops out at commit `355ae75f` for INVFIX report work. The codebase still reflects the original cross-module report pass plus the earlier post-audit fixes, but the remaining INVFIX-08 carryover did not land.

The following earlier fixes must be preserved:

- `report-hub.tsx` links use the live dashboard route shape.
- `getCrossModuleClients()` computes combined revenue before slicing to the visible top-N rows.
- report CSV exports use centralized escaping/serialization in `report-actions.ts`.

Your job in this session is to **actually close the remaining INVFIX-08 carryover**.

## Hard Scope

This session is **INVFIX-08 carryover closure only**.

Do **not** begin INVFIX-09 unless all carryover below is fully implemented, validated, and TypeScript-clean relative to baseline. If the carryover is still larger than expected, stop after the report work and document what remains. Do not force progress into email-system work.

## Mission

Finish the report overhaul to the level described in the master guide, using the existing Session 9 report implementation as the baseline instead of rewriting it.

## Confirmed Gaps That Still Exist
...
Do not invent COGS data if the platform does not store it. If gross margin requires assumptions, make those assumptions explicit in code/UI and keep them conservative.
...
Note: a reusable AI risk hook already exists in the repo (`ai-actions.ts` + `client-risk-badge.tsx`). Reuse it only if the integration is safe and lightweight.
...
- add a real PDF export path, or
- if browser print to PDF is the established platform pattern, standardize the report surfaces and document that decision clearly in code and memory bank instead of pretending a separate PDF pipeline exists
...
- exports are robust and not broken by commas or quotes in data
