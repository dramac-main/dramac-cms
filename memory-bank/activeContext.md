...
**Full white-label audit: removed ALL upstream supplier brand exposure from agency and client-facing UI.**

ResellerClub (domain/email provisioning) and Titan (email hosting) are hidden suppliers — agencies/clients must never see their names.

#### Deliverables:

1. **ResellerClub brand removal (6 fixes)**: Balance error now shows generic "provisioning capacity issue", removed "Live from ResellerClub", "Registered via ResellerClub", "via ResellerClub" from email page, "Wholesale" → "Cost" in TLD pricing, "Order ID" → "Registration ID"

2. **Titan brand removal (5 fixes)**: Plan name fallbacks "Titan Email" → "Business Email", aria-label "Open Titan webmail" → "Open webmail", portal text "Titan Mail webmail" → "webmail", removed "app.titan.email" text reference. Functional URLs kept (needed for service).

3. **Package card styling**: Added `bg-card` to email plan cards for proper contrast

4. **Audit findings — clean**: No simulated auth codes (false positive), transfer auth code implementation is proper, DNS records are infrastructure values not branding

#### Commit: `1c3297f5` — pushed to main

### Previous: INVFIX Session 11 Verification — INVFIX-08 Improved, Still Not Fully Closed

### INVFIX Session 11 (April 20, 2026 verification)

**A real Session 11 closure commit exists locally (`b4731f61`), and it closes most of the previously identified report carryover. However, a deeper code audit against the master guide shows INVFIX-08 is still not fully spec-complete.**

#### What Session 11 definitely landed

- `getCashFlowReport()` now includes invoicing payments plus e-commerce orders and booking payments, with running net position and source-level cash-in breakdown.
- `getRevenueTrendsComparison()` now exists and powers period-over-period totals, growth indicators, and top-10-vs-other client segmentation in `revenue-trends-report.tsx`.
- `getProfitAndLoss()` now exposes `grossMargin`, `grossMarginPercent`, and `ytdComparison`, and `pnl-report.tsx` renders those values.
- `getARAgingReport()` now computes `weightedDSO`, and `getARAgingInvoices()` enables bucket drilldowns from `ar-aging-report.tsx`.
- `getTaxSummary()` now returns `byFilingPeriod`, and `tax-summary-report.tsx` renders the filing-period table.
- `getExpenseReport()` now returns `topVendors`, `budgetComparison`, and `yoyComparison`, and `expense-report.tsx` renders them.
- `report-types.ts` now documents the standardized export decision: CSV plus browser Print to PDF.

#### Remaining INVFIX-08 gaps after the verification audit

1. **AR aging collection probability is still missing.**
   The master guide still calls for collection probability based on AI risk scores. The reusable AI hook exists (`getClientRiskScore()` / `client-risk-badge.tsx`), but it is not integrated into `ar-aging-report.tsx` or the AR report data types.
2. **Projected cash flow is still not integrated into the report surface.**
   `getCashFlowReport()` hard-codes `hasProjectedData: false`, and the report UI does not surface forecast data. A separate `CashFlowForecastChart` exists under the insights surface, but it is not wired into the cash flow report.
3. **Tax CSV export is not yet filing-oriented.**
   `getTaxSummary()` computes `byFilingPeriod`, but `exportReportCSV("tax")` still exports only the by-rate breakdown plus totals, not the filing-period structure needed for tax filing use.
4. **Cross-module report health scorecards are still absent.**
   `cross-module-report.tsx` includes revenue and client activity tabs, but it does not yet implement the module health scorecards described in the master guide.

#### Important repo-state note

- `main` is currently **ahead of `origin/main` by 1 commit** (`b4731f61`).
- There are also current local modifications in several report files, but the report-specific diffs are formatting-only and do not change the audit outcome above.
...
Major feature batch addressing blog editor incompleteness, missing agency-side support ticket system, email notification gaps, and missing default chat departments.

### Blog Editor Enhancement

- Added CodeBlockLowlight with syntax highlighting (installed @tiptap/extension-code-block-lowlight + lowlight)
- Added Media Library image picker via tabbed dialog (URL paste + Media Library)
- Added YouTube/Vimeo embed dialog with auto-URL conversion
- Added alt text support for images
- Added code block toggle button in toolbar
- PostForm now passes agencyId/siteId to PostEditor for media library access

### Blog Service Fix

- Added 5 missing fields to `createPost()`: metaKeywords, ogImageUrl, canonicalUrl, allowComments, isFeatured

### Agency Support Ticket System (NEW)

- Created `src/lib/support/ticket-service.ts` with 7 functions: getAgencyContext, getAgencyTickets, getAgencyTicket, replyToTicket, updateAgencyTicketStatus, assignTicket, getAgencyTicketStats
- Created tickets list page (`/dashboard/tickets`) with TicketsDashboard component (stats, search, filters)
- Created ticket detail page (`/dashboard/tickets/[ticketId]`) with TicketDetailView component (message thread, reply form, status controls)
- Added "Support Tickets" to dashboard sidebar navigation

### Email Notifications

- Added 3 new email types: support_ticket_created, support_ticket_replied, support_ticket_closed
- Added HTML/text templates for all 3 support email types
- Wired email notification on client ticket creation (portal support-service.ts → agency)
- Wired email notification on agent reply and status change (ticket-service.ts → client)

### Default Industry Departments

- Added `seedDefaultDepartments(siteId)` to site creation flow in `installCoreModules()`
- Seeds 5 industry-standard departments: General, Sales, Support, Billing, Customer Service
- Runs automatically when live-chat module is installed for a new site

### Files Created (5)

1. `src/lib/support/ticket-service.ts` — Agency ticket service (7 functions)
2. `src/app/(dashboard)/dashboard/tickets/page.tsx` — Tickets list page
3. `src/app/(dashboard)/dashboard/tickets/[ticketId]/page.tsx` — Ticket detail page
4. `src/components/support/tickets-dashboard.tsx` — Full dashboard with stats/search/filters
5. `src/components/support/ticket-detail-view.tsx` — Detail view with message thread + reply

### Files Modified (11)

1. `src/components/blog/post-editor.tsx` — CodeBlockLowlight, media picker, embed, tabbed image dialog
2. `src/components/blog/post-form.tsx` — Pass agencyId/siteId to PostEditor
3. `src/lib/blog/post-service.ts` — Added 5 missing fields to createPost()
4. `src/config/navigation.ts` — Added Ticket import + "Support Tickets" nav item
5. `src/lib/email/email-types.ts` — Added 3 support ticket email types
6. `src/lib/email/templates.ts` — Added 3 support ticket email templates
7. `src/lib/portal/support-service.ts` — Added email notification on ticket creation
8. `src/lib/actions/sites.ts` — Added seedDefaultDepartments() + call in installCoreModules
9. `package.json` — Added @tiptap/extension-code-block-lowlight + lowlight deps
10. `pnpm-lock.yaml` — Updated lockfile
11. `src/lib/portal/portal-billing-service.ts` — (from prior session)
...
#### Action Executor — 3 New Categories (action-executor.ts)

- **E-Commerce actions (10)**: update_order_status, add_order_note, add_shipment, create_refund, adjust_stock, update_quote_status, send_quote, send_quote_reminder, convert_quote_to_order, notify_quote_created
- **Booking actions (5)**: create_appointment, update_appointment, update_status, cancel_appointment, create_reminder
- **Chat actions (5)**: send_message, assign_conversation, resolve_conversation, close_conversation, update_tags
- All wired to real module server actions via proper imports
- TypeScript type casts for InventoryMovementType, QuoteStatus, AppointmentStatus, CancelledBy, ReminderType, MessageContentType

#### Action Types Registry — 3 New Sections (action-types.ts) [committed in prior session]

- `ecommerce` category: 9 ActionDefinitions with full input/output schemas
- `booking` category: 5 ActionDefinitions with full input/output schemas
- `chat` category: 5 ActionDefinitions with full input/output schemas
- ACTION_CATEGORIES array updated (now 11 categories total)

#### Cross-Module Workflow Templates — 14 New (templates.ts)

- **Order → CRM Contact & Deal**: New order creates/finds CRM contact + deal
- **Order Shipped → Multi-Channel Notification**: Email + CRM activity log
- **Quote Accepted → Convert to Order**: Auto-convert + email + team notification
- **Quote Follow-Up Reminder Sequence**: 2-day + 7-day auto-reminder drip
- **Quote Rejected → Team Review**: CRM log + in-app alert
- **Booking → CRM Contact & Activity**: Find/create contact + log booking
- **Booking Cancelled → Follow-Up**: Reschedule email + CRM log
- **Booking Confirmed → Chat Message**: Chat confirmation + 24h reminder
- **Chat → CRM Contact**: Find/create visitor contact + activity log
- **Chat Resolved → Satisfaction Follow-Up**: 1h delay → feedback email + CRM log
- **Low Stock → Slack + Email Alert**: Multi-channel inventory alerts
- **Refund Processed → CRM & Notifications**: Email + CRM + in-app
- **Deal Won → Send Quote**: CRM deal stage change triggers quote send
- **VIP Customer Chat → Priority Escalation**: CRM tag check → auto-assign senior agent

#### Key Findings

- **Email, notification, webhook, data, transform, flow actions were already FULLY IMPLEMENTED** — not stubs as previously assumed
- Template gallery UI was already fully wired to templates.ts — no changes needed
- `createWorkflowFromTemplate` in automation-actions.ts creates workflow + steps + event subscription

### Automation Module Architecture Summary

| Component             | File                 | Status                                                                                                |
| --------------------- | -------------------- | ----------------------------------------------------------------------------------------------------- |
| Action Executor       | action-executor.ts   | ✅ 10 categories (crm, ecommerce, booking, chat, email, notification, webhook, data, transform, flow) |
| Action Types Registry | action-types.ts      | ✅ 11 categories with full schemas                                                                    |
| Event Processor       | event-processor.ts   | ✅ Fully working (log + immediate trigger)                                                            |
| Execution Engine      | execution-engine.ts  | ✅ Variable resolution + condition evaluation                                                         |
| Templates             | templates.ts         | ✅ 36 templates (22 original + 14 new cross-module)                                                   |
| Template Gallery UI   | template-gallery.tsx | ✅ Fully wired, no changes needed                                                                     |
| Workflow Builder      | workflow-builder.tsx | ✅ Visual step editor                                                                                 |
| DB Tables             | 10 tables            | ✅ All present in Supabase                                                                            |

---

## Previous Focus: CRM Overhaul Complete + Full TypeScript Clean ✅ (commit 833135b7)
...
## Previous Focus: Internal Notes Preview Leak + @Mention Fix + Order Lifecycle Audit ✅ (commit 71ada0e5)
