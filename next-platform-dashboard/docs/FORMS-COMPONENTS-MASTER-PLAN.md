...
| --------------- | -------------------------------------------- | -------------------------------------------- | ----------- |
| **Form**        | L17423 (`export interface FormProps`)        | L17504 (`export function FormRender`)        | 71          |
| **FormField**   | L17827 (`export interface FormFieldProps`)   | L17957 (`export function FormFieldRender`)   | 67          |
| **ContactForm** | L18418 (`export interface ContactFormProps`) | L18448 (`export function ContactFormRender`) | 30          |
| **Newsletter**  | L18756 (`export interface NewsletterProps`)  | L18772 (`export function NewsletterRender`)  | 14          |

#### core-components.ts

| Component       | `defineComponent({` | `type:` line | Fields | Field Groups | defaultProps |
| --------------- | ------------------- | ------------ | ------ | ------------ | ------------ |
| **Form**        | L12692              | L12693       | 50+    | 13           | L13099       |
| **FormField**   | L13130              | L13131       | 45+    | 12           | L13481       |
| **ContactForm** | L13509              | L13510       | 7      | 0 (flat)     | L13541       |
| **Newsletter**  | L13555              | L13556       | 6      | 0 (flat)     | L13594       |
...
| `FormBlock`         | `"Form"`        | L494                  |
| `FormSection`       | `"Form"`        | L495                  |
| `ContactForm`       | `"ContactForm"` | L535 (identity alias) |
| `Newsletter`        | `"Newsletter"`  | L555 (identity alias) |

**Keyword-based mappings** (lowercase slug → component):

- `location`, `findus`, `visitreuneus` → `"ContactForm"` (L907-L909)
- `contactinfo`, `contactdetails`, `reachout` → `"ContactForm"` (L929-L931)

**`KNOWN_REGISTRY_TYPES` Set** (L774): `"ContactForm"` at L800, `"Newsletter"` at L801, `"Form"` at L802, `"FormField"` at L803.

> ✅ **All 4 form components are in KNOWN_REGISTRY_TYPES.** Form also has typeMap aliases (FormBlock, FormSection). FormField has no typeMap aliases — the AI must generate it as a child of Form, not standalone.
...
renderer.tsx reads component.type, looks up render function from registry
  ↓
renderer.tsx L224-228: injects siteId into ALL components:
    let injectedProps = {
      ...component.props,
...
| --------------------- | --------------------------- | ---------------- | ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| **FormRender**        | `renders.tsx` L17423→L17504 | 71               | ⚠️ Generic    | No `/api/forms/submit` integration. Generic `<form>` wrapper with `action`/`method`/`onSubmit`. Styling-rich (13 field groups — layout, header, submit button, animation, messages) but no built-in submission pipeline. Accepts children (FormField components).                                                                                                       |
| **FormFieldRender**   | `renders.tsx` L17827→L17957 | 67               | ✅ Strong     | Comprehensive field component: 17 input types, prefix/suffix, icons, validation, password toggle, char count, clear button. Well-structured with correct variant maps. This is the best component in the set.                                                                                                                                                           |
| **ContactFormRender** | `renders.tsx` L18418→L18448 | 30               | ✅ Functional | **Only component with built-in submission.** POSTs to `/api/forms/submit` with siteId + honeypot. Has success/error states. BUT: `emailTo` prop is sent in data as `_emailTo` but the API **never reads it** — emails only come from `form_settings.notify_emails`. If no `form_settings` row exists, `notify_emails` defaults to `[]` — **no notifications get sent**. |
| **NewsletterRender**  | `renders.tsx` L18756→L18772 | 14               | ⚠️ Incomplete | Uses standard HTML `<form action={action} method="POST">` — **NO** fetch to `/api/forms/submit`. No JS submission handler. No success/error states. No honeypot. No siteId usage. On published sites, `action` defaults to `"#"` — form submits to current page and does nothing. **Completely non-functional out of the gate.**                                        |
...
| 5   | **FormField has no typeMap aliases**                 | 🟡 Medium   | FormField    | FormField is in `KNOWN_REGISTRY_TYPES` but has no typeMap aliases. AI can only generate it as a child of Form, not directly. Form has `FormBlock`/`FormSection` aliases and is fully converter-integrated.                                              |
...
| --------------- | ------------------------------------------------- | --------------------- | ------------ | ------------------------------------------------------------------- |
| **Form**        | ✅ FormBlock/FormSection (L494-L495)              | 50+ fields, 13 groups | 71 props     | ✅ Fields well-matched to props, AI-reachable via aliases           |
| **FormField**   | ⚠️ In KNOWN_REGISTRY_TYPES but no typeMap aliases | 45+ fields, 12 groups | 67 props     | ⚠️ Reachable as child of Form but no standalone aliases             |
...
| ------------------- | -------------------------------------------------------------------------------------- |
| **Type**            | `"Form"`                                                                               |
| **File**            | `renders.tsx` L17423 (interface) → L17504 (export)                                     |
| **Registry**        | `core-components.ts` L12692 (`defineComponent`)                                        |
| **Metadata**        | `component-metadata.ts` L581                                                           |
| **Converter**       | ✅ `FormBlock`/`FormSection` in typeMap (L494-L495), in `KNOWN_REGISTRY_TYPES` at L802 |
| **Category**        | `"forms"`                                                                              |
...
### 4.2 Props Interface Summary (L17423-L17502)
...
The Form registry at L12692-L13129 is **comprehensive** — 50+ fields across 13 field groups with proper `type`, `label`, `options`, and `defaultValue` definitions. All field names match the render prop names.

**defaultProps** (L13099-L13112):
...
| ------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| **Type**            | `"FormField"`                                                                                                                          |
| **File**            | `renders.tsx` L17827 (interface) → L17957 (export)                                                                                     |
| **Registry**        | `core-components.ts` L13130 (`defineComponent`)                                                                                        |
| **Metadata**        | `component-metadata.ts` L593                                                                                                           |
| **Converter**       | ⚠️ In `KNOWN_REGISTRY_TYPES` at L803 but no typeMap aliases                                                                            |
| **Category**        | `"forms"`                                                                                                                              |
| **Props**           | 67 (interface)                                                                                                                         |
...
### 5.2 Props Interface Summary (L17827-L17956)
...
| Issue                                  | Details                                                     | Fix Required                                                                          |
| -------------------------------------- | ----------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| In KNOWN_REGISTRY_TYPES but no aliases | AI knows the type but has no alias mappings for it          | Add typeMap aliases (FormFieldBlock, InputField, FormInput) — as child-only component |
...
| ------------------- | ------------------------------------------------------------ |
| **Type**            | `"ContactForm"`                                              |
| **File**            | `renders.tsx` L18418 (interface) → L18448 (export)           |
| **Registry**        | `core-components.ts` L13509 (`defineComponent`)              |
| **Metadata**        | `component-metadata.ts` L604                                 |
| **Converter**       | ✅ 10+ aliases in typeMap, in `KNOWN_REGISTRY_TYPES` at L800 |
| **Category**        | `"forms"`                                                    |
| **Props**           | 30 (interface)                                               |
| **Registry Fields** | 7 (flat, no groups)                                          |
| **Submission**      | ✅ Built-in fetch to `/api/forms/submit`                     |

### 6.2 Props Interface (L18418-L18447)
...
### 6.3 Submission Flow (L18489-L18561)
...
| ------------------- | ---------------------------------------------------------------------------------------- |
| **Type**            | `"Newsletter"`                                                                           |
| **File**            | `renders.tsx` L18756 (interface) → L18772 (export)                                       |
| **Registry**        | `core-components.ts` L13555 (`defineComponent`)                                          |
| **Metadata**        | `component-metadata.ts` L616                                                             |
| **Converter**       | ✅ 7+ aliases in typeMap, in `KNOWN_REGISTRY_TYPES` at L801                              |
| **Category**        | `"forms"`                                                                                |
| **Props**           | 14 (interface)                                                                           |
| **Registry Fields** | 6 (flat, no groups)                                                                      |
| **Submission**      | ❌ Non-functional: `<form action={action} method="POST">` where action defaults to `"#"` |

### 7.2 Props Interface (L18756-L18771)
...
Wait — checking the registry at L13594-L13599:
...
async function sendNotifications(
  ...
  console.log(
    "[FormSubmit] No notification recipients found for site:",
    site.id,
  );
}
```
...
<input
  ...
  aria-invalid={!!error}
  aria-describedby={
    helpText ? `${fieldId}-help` : error ? `${fieldId}-error` : undefined
  }
  {...otherProps}
/>;
...
const isDark = backgroundColor ? isColorDark(backgroundColor) : false;
```
...
| --------------- | -------------------------------------- | --------------------- | --------------------------------------------------------------------------------------------------- |
| **Form**        | ✅ FormBlock/FormSection               | ✅ If manually placed | title, description, submitText, successMessage, errorMessage, layout, gap, padding, backgroundColor |
| **FormField**   | ⚠️ In KNOWN_REGISTRY_TYPES, no aliases | ✅ If manually placed | ❌ No ai.canModify defined                                                                          |
...
The converter already has special handling for Newsletter at L2027-L2039:
...
if (type === "Newsletter") {
  return {
    ...
    buttonText:
      props.buttonText || props.ctaText || props.submitText || "Subscribe",
    placeholder: props.placeholder || "Enter your email",
  };
}
```

> Note: This converter handling correctly maps `subtitle` → `description` and falls back through multiple button text sources (`buttonText`, `ctaText`, `submitText`). However, it does NOT map `layout` → `variant` — the **registry** still uses the wrong field names (submitText, layout, subtitle). The converter's `...props` spread passes whatever the AI generates, but sidebar editing uses registry field names which don't match render prop names.

### 12.4 Required AI Enhancements

1. **Add FormField typeMap aliases** — allow AI to generate form fields (FormFieldBlock, InputField, FormInput)
2. **Enhance ContactForm ai.canModify** to include styling props
3. **Enhance Newsletter ai.canModify** to include styling props
...
**Add these fields to ContactForm's `defineComponent` at L13509:**
...
**Replace Newsletter `defineComponent` fields at L13556:**
...
### 13.3 FormField — Add to Converter typeMap

**Add to `typeMap` in converter.ts (FormField only — Form already has FormBlock/FormSection):**

```typescript
// FormField aliases (child of Form only)
...
| --------------- | --------------------------------------------------------------- | ----------------------------------------------------------- | ------------------------------------------- |
| **Form**        | Add siteId, formId, enablePlatformSubmission fields             | Already in typeMap + KNOWN_REGISTRY_TYPES                   | Add platform submission handler             |
| **FormField**   | Add aria props to AI hints                                      | Add typeMap aliases (FormFieldBlock, InputField, FormInput) | Add ARIA attributes                         |
...
| 1.2  | `renders.tsx`                | L18772-L18944 (NewsletterRender) | Rewrite: add `siteId` prop, `handleSubmit` with fetch to `/api/forms/submit`, honeypot, success/error states, display `successMessage`                          |
| 1.3  | `renders.tsx`                | L18756 (NewsletterProps)         | Add `siteId?: string` to interface                                                                                                                              |
| 1.4  | `core-components.ts`         | L13555 (Newsletter)              | Fix field name mismatches: `submitText` → `buttonText`, `layout` → `variant`, add `subtitle` alias or rename to `description`. Add `"card"` to variant options. |
| 1.5  | `core-components.ts`         | L13594 (Newsletter defaultProps) | Fix key names to match render props                                                                                                                             |
...
| Task | File                 | Lines                   | Change                                                           |
| ---- | -------------------- | ----------------------- | ---------------------------------------------------------------- |
| 2.1  | `core-components.ts` | L13509 (ContactForm)    | Add 16 missing fields (labels, styling, colours)                 |
| 2.2  | `core-components.ts` | L13555 (Newsletter)     | Add `backgroundColor`, `buttonColor`, `textColor`, `size` fields |
| 2.3  | `core-components.ts` | L13509 (ContactForm ai) | Expand `canModify` to include styling props                      |
| 2.4  | `core-components.ts` | L13555 (Newsletter ai)  | Expand `canModify` to include styling props                      |
...
| Task | File                 | Lines               | Change                                                               |
| ---- | -------------------- | ------------------- | -------------------------------------------------------------------- |
| 3.1  | `converter.ts`       | typeMap             | Add FormField aliases (FormFieldBlock, InputField, FormInput)        |
| 3.2  | `renders.tsx`        | L17423 (FormProps)  | Add `siteId`, `formId`, `enablePlatformSubmission` props             |
| 3.3  | `renders.tsx`        | L17504 (FormRender) | Add platform submission handler (when enablePlatformSubmission=true) |
| 3.4  | `core-components.ts` | L12692 (Form)       | Add `siteId`, `formId`, `enablePlatformSubmission` fields            |
...
│  2. Inject siteId: { ...component.props, siteId }  (L224)   │
...
│                                                              │
│  FormRender (L17504)          FormFieldRender (L17957)       │
│  └── Generic container         └── 17 input types            │
│      action/method/onSubmit        label/validation/styling  │
│                                                              │
│  ContactFormRender (L18448)   NewsletterRender (L18772)      │
...
_Last updated: March 2026 — All line numbers re-verified and corrected_
_Verified against source: All line numbers grep-verified March 2026_
_Total form-related code: ~4,470 lines across 14 files_
