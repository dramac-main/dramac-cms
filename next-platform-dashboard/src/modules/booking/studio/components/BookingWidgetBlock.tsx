...
const DEMO_SERVICES: ServiceItem[] = [
  ...
  },
  {
    ...
    description: "Intensive deep muscle treatment.",
    duration: 90,
    price: 120,
    currency: DEFAULT_CURRENCY,
    category: "Massage",
  },
  {
    ...
    description: "Premium facial with skincare.",
    duration: 45,
    price: 65,
    currency: DEFAULT_CURRENCY,
    category: "Skincare",
  },
  {
    id: "4",
    name: "Hair Styling",
    description: "Professional hair styling consultation.",
    duration: 30,
    price: 45,
    currency: DEFAULT_CURRENCY,
    category: "Hair",
...
export function BookingWidgetBlock({
  ...
  const steps = useMemo(() => {
    const s = [];
    if (showServiceStep)
      s.push({ id: "service", label: stepServiceLabel, icon: Tag });
    if (showStaffStep)
      s.push({ id: "staff", label: stepStaffLabel, icon: User });
  ...
  // ── Real Data Hooks ──────────────────────────────────────────────────────
  const { services: realServices, isLoading: loadingServices } =
    useBookingServices(siteId || "");
  const { staff: realStaff, isLoading: loadingStaff } = useBookingStaff(
    siteId || "",
  );
  ...
  } = useBookingSlots(siteId || "", {
    serviceId: selectedService?.id,
    date: selectedDate || undefined,
    staffId: selectedStaff?.id,
  });
  const { createBooking, isSubmitting: isCreatingBooking } = useCreateBooking(
    siteId || "",
  );

  // Track recently booked slots for immediate UI feedback (belt-and-suspenders)
  const [recentlyBookedSlots, setRecentlyBookedSlots] = useState<Set<string>>(
    new Set(),
  );
  ...
  return (
    ...
    >
          ...
          {stepIndicatorStyle === "progress-bar" ? (
            ...
            <div
              style={{
                ...
                alignItems: "center",
                justifyContent: "center",
                gap: stepIndicatorStyle === "pills" ? "4px" : "8px",
              }}
            >
              {steps.map((step, idx) => {
      ...
      {/* Step Content */}
      <div className="p-3 sm:p-5" style={{ minHeight: "200px" }}>
        {/* SERVICE STEP */}
          ...
          >
            {showNameField && (
              <div>
                ...
                >
                  <User style={{ width: 14, height: 14, opacity: 0.6 }} /> Full
                  Name{" "}
                  {nameRequired && <span style={{ color: errorClr }}>*</span>}
                </label>
                <input
                  type="text"
                  value={formData.name || ""}
            ...
            {showEmailField && (
              <div>
                ...
                >
                  <Mail style={{ width: 14, height: 14, opacity: 0.6 }} /> Email{" "}
                  {emailRequired && <span style={{ color: errorClr }}>*</span>}
                </label>
                <input
                  type="email"
            ...
            {showPhoneField && (
              <div>
                ...
                >
                  <Phone style={{ width: 14, height: 14, opacity: 0.6 }} />{" "}
                  Phone
                </label>
                <input
                  type="tel"
            ...
            {showNotesField && (
              <div>
                ...
                >
                  <FileText style={{ width: 14, height: 14, opacity: 0.6 }} />{" "}
                  Notes
                </label>
                <textarea
          ...
          {currentStepId === "confirm" ? (
            ...
            <button
              ...
              style={{
                padding: "10px 24px",
                borderRadius: buttonBorderRadius,
                backgroundColor: canGoNext() ? btnBg : `${btnBg}40`,
                color: btnTxt,
                border: "none",
                fontSize: buttonFontSize,
                fontWeight: buttonFontWeight,
...
export const bookingWidgetDefinition: ComponentDefinition = {
  ...
  description:
    "All-in-one multi-step booking wizard — 50+ customization options",
  ...
  ai: {
    description:
      "Complete multi-step booking wizard — fully customizable with 50+ properties",
    canModify: [
      "title",
      "subtitle",
      "stepIndicatorStyle",
