...
export async function getCampaigns(
  ...
  try {
    ...
    return {
      campaigns: mapRecords<Campaign>(data || []),
      total: count || 0,
      error: null,
    };
  } catch (error) {
  ...
  try {
    ...
    const { data: campaign, error } = await (supabase as any)
      ...
      .insert({
        ...
        utm_source:
          data.utmSource || data.name.toLowerCase().replace(/\s+/g, "-"),
        utm_medium: data.utmMedium || "social",
        utm_campaign:
          data.utmCampaign || data.name.toLowerCase().replace(/\s+/g, "-"),
        total_posts: 0,
        total_impressions: 0,
        total_engagement: 0,
        total_clicks: 0,
    ...
    return {
      campaign: campaign ? mapRecord<Campaign>(campaign) : null,
      error: null,
    };
  } catch (error) {
...
export async function updateCampaign(
  ...
  try {
    ...
    if (updates.utmSource !== undefined)
      updateData.utm_source = updates.utmSource;
    if (updates.utmMedium !== undefined)
      updateData.utm_medium = updates.utmMedium;
    if (updates.utmCampaign !== undefined)
      updateData.utm_campaign = updates.utmCampaign;
...
export async function getCampaignAnalytics(campaignId: string): Promise<{
  ...
  try {
    ...
    const totalImpressions =
      posts?.reduce(
        (sum: number, p: any) => sum + (p.total_impressions || 0),
        0,
      ) || 0;
    const totalEngagement =
      posts?.reduce(
        (sum: number, p: any) => sum + (p.total_engagement || 0),
        0,
      ) || 0;
    const totalClicks =
      posts?.reduce((sum: number, p: any) => sum + (p.total_clicks || 0), 0) ||
      0;
    ...
    if (goals.impressions) {
      goalProgress.impressions = {
        ...
        percentage: Math.min(
          100,
          Math.round((totalImpressions / goals.impressions) * 100),
        ),
      };
    }
    ...
    if (goals.engagement) {
      goalProgress.engagement = {
        ...
        percentage: Math.min(
          100,
          Math.round((totalEngagement / goals.engagement) * 100),
        ),
      };
    }
    ...
    if (goals.clicks) {
      goalProgress.clicks = {
        ...
        percentage: Math.min(
          100,
          Math.round((totalClicks / goals.clicks) * 100),
        ),
      };
    }
...
export async function updateCampaignStats(
  ...
  try {
    ...
    const totalClicks =
      stats?.reduce((sum: number, p: any) => sum + (p.total_clicks || 0), 0) ||
      0;
