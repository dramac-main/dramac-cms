...
export async function getPosts(
  ...
  try {
    ...
    return {
      posts: mapRecords<SocialPost>(data || []),
      total: count || 0,
      error: null,
    };
  } catch (error) {
...
export async function createPost(
  ...
  data: {
    ...
    platformContent?: Record<
      SocialPlatform,
      { content: string; media?: PostMedia[] }
    >;
...
export async function updatePost(
  ...
  try {
    ...
    if (updates.platformContent !== undefined)
      updateData.platform_content = updates.platformContent;
    if (updates.targetAccounts !== undefined)
      updateData.target_accounts = updates.targetAccounts;
    ...
    if (updates.campaignId !== undefined)
      updateData.campaign_id = updates.campaignId;
    if (updates.contentPillar !== undefined)
      updateData.content_pillar = updates.contentPillar;
    if (updates.firstComment !== undefined)
      updateData.first_comment = updates.firstComment;
...
export async function publishPostNow(
  ...
  try {
    ...
    const someSucceeded = Object.values(results).some(
      (r) => r.status === "success",
    );
...
export async function addToQueue(
  ...
  try {
    ...
    const { data: nextSlot } = await (supabase as any).rpc(
      "get_social_next_queue_slot",
      { p_queue_id: queueId },
    );
    ...
    if (!nextSlot) {
      return {
        success: false,
        scheduledAt: null,
        error: "No available slots in queue",
      };
    }
...
export async function duplicatePost(
  ...
  try {
    ...
    return {
      post: newPost ? mapRecord<SocialPost>(newPost) : null,
      error: null,
    };
  } catch (error) {
