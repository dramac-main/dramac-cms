...
export async function createWorkflow(
  ...
  try {
    ...
    await emitEvent("automation", siteId, "automation.workflow_created", {
...
export async function getWorkflows(
  ...
  try {
    ...
    return {
      success: true,
      data: data as unknown as Workflow[],
      count: count || 0,
    };
  } catch (error) {
    return {
...
export async function updateWorkflow(
  ...
  try {
    ...
    if (data.trigger_config !== undefined)
      updateData.trigger_config = data.trigger_config as unknown as Json;
    if (data.is_active !== undefined) updateData.is_active = data.is_active;

    // Build update query with defense-in-depth site_id scoping
    let updateQuery = supabase
      .from("automation_workflows")
      .update(updateData)
      .eq("id", workflowId);

    // Defense-in-depth: scope to site_id when available (in addition to RLS)
    if (currentWorkflow?.site_id) {
      updateQuery = updateQuery.eq("site_id", currentWorkflow.site_id);
    }

    const { data: workflow, error } = await updateQuery.select("*").single();
    ...
    const triggerConfig = (data.trigger_config ||
      currentWorkflow?.trigger_config) as TriggerConfig;
    ...
    if (data.is_active === true && siteId) {
      ...
      if (triggerType === "event" && triggerConfig?.event_type) {
        console.log(
          `[Automation] Creating event subscription for ${triggerConfig.event_type}`,
        );
        ...
        if (subError) {
          console.error(
            "[Automation] Failed to create event subscription:",
            subError,
          );
        } else {
          console.log(
            `[Automation] ✅ Subscription created for ${triggerConfig.event_type}`,
          );
        }
      }
      ...
      console.log(
        `[Automation] Deactivating event subscriptions for workflow ${workflowId}`,
      );
...
export async function deleteWorkflow(
  ...
  try {
    ...
    // Defense-in-depth: scope delete to site_id when available (in addition to RLS)
    ...
    if (workflow?.site_id) {
      deleteQuery = deleteQuery.eq("site_id", workflow.site_id);
    }
...
export async function activateWorkflow(
  ...
  try {
    ...
    // Update is_active to true (defense-in-depth: scope to site_id)
    const { error: updateError } = await supabase
      .from("automation_workflows")
      .update({ is_active: true })
      .eq("id", workflowId)
      .eq("site_id", workflow.site_id);
    ...
    if (workflow.trigger_type === "event" && triggerConfig?.event_type) {
      ...
      if (subError) {
        ...
        console.log(
          `[Automation] Created subscription for ${triggerConfig.event_type}`,
        );
      }
    }
    ...
    if (workflow.trigger_type === "schedule" && triggerConfig?.cron) {
      await supabase.from("automation_scheduled_jobs").upsert(
        {
          site_id: workflow.site_id,
          workflow_id: workflowId,
  ...
  try {
    ...
    if (stepId.startsWith("temp-")) {
      return {
        success: false,
        error:
          "Cannot update temporary step - please wait for step to be created",
      };
    }
    ...
    if (data.action_config !== undefined)
      updateData.action_config = data.action_config as unknown as Json;
    if (data.position !== undefined) updateData.position = data.position;
    if (data.condition_config !== undefined)
      updateData.condition_config = data.condition_config as unknown as Json;
    if (data.delay_config !== undefined)
      updateData.delay_config = data.delay_config as unknown as Json;
    ...
    if (data.retry_delay_seconds !== undefined)
      updateData.retry_delay_seconds = data.retry_delay_seconds;
    if (data.input_mapping !== undefined)
      updateData.input_mapping = data.input_mapping as unknown as Json;
    if (data.output_key !== undefined)
      updateData.output_key = data.output_key || null;
...
export async function createConnection(
  ...
  try {
    ...
    return {
      success: true,
      data: connection as unknown as AutomationConnection,
    };
  } catch (error) {
    return {
      success: false,
...
export async function getConnections(siteId: string): Promise<{
  ...
  try {
    ...
    return {
      success: true,
      data: connections as unknown as AutomationConnection[],
    };
  } catch (error) {
    return {
      success: false,
...
export async function updateConnection(
  ...
  try {
    ...
    if (data.credentials !== undefined)
      updateData.credentials = data.credentials as unknown as Json;
...
export async function testConnection(
  ...
  try {
    ...
    const credentials = connection.credentials as Record<
      string,
      unknown
    > | null;
    ...
    // Update last tested timestamp (scoped to site_id for safety)
    await supabase
      .from("automation_connections")
      .update({
        status: "active",
...
export async function getWebhookEndpoints(
  ...
  try {
    ...
    const { data: endpoints, error } = await supabase
      .from("automation_webhook_endpoints")
      .select(
        "id, site_id, workflow_id, endpoint_path, is_active, total_calls, last_called_at, created_at",
      )
      .eq("workflow_id", workflowId);
...
export async function subscribeToEvent(
  ...
  try {
    ...
    return {
      success: false,
      error:
        error instanceof Error ? error.message : "Failed to subscribe to event",
    };
  }
}
 ...
 */
export async function getAutomationStats(siteId: string): Promise<{
  success: boolean;
  data?: {
...
export async function getAutomationAnalytics(
  ...
  try {
    ...
    const failedExecutions = executionList.filter(
      (e) => e.status === "failed",
    ).length;
    const successRate =
    ...
    const completedExecutions = executionList.filter(
    ...
    const executionTrend =
      previousExecutions && previousExecutions > 0
        ? ((totalExecutions - previousExecutions) / previousExecutions) * 100
        : 0;

    // Executions by day
    const executionsByDay: Array<{
      date: string;
      total: number;
      successful: number;
      failed: number;
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      executionsByDay.push({
        ...
        successful: dayExecutions.filter((e) => e.status === "completed")
          .length,
    ...
    const workflowStats = workflowList
      .map((w) => {
        const wfExecutions = executionList.filter(
          (e) => e.workflow_id === w.id,
        );
        const wfSuccess = wfExecutions.filter(
          (e) => e.status === "completed",
        ).length;
        return {
          id: w.id,
          ...
          successRate:
            wfExecutions.length > 0
              ? (wfSuccess / wfExecutions.length) * 100
              : 0,
        };
      })
...
export async function createWorkflowFromTemplate(
  ...
  try {
    ...
    if (workflowError) {
      console.error(
        "[Automation] Create workflow from template error:",
        workflowError,
      );
    ...
    if (stepsError) {
      console.error(
        "[Automation] Create steps from template error:",
        stepsError,
      );
    ...
    console.error(
      "[Automation] Create workflow from template exception:",
      error,
    );
    return {
      success: false,
