...
export async function processPendingEvents(
  ...
  if (fetchError || !events?.length) {
    return {
      processed: 0,
      triggered: 0,
      errors: fetchError ? [fetchError.message] : [],
    };
  }
...
async function handleEvent(
  ...
  for (const sub of subscriptions) {
    ...
    const workflow = subscription.workflow as {
      id: string;
      is_active: boolean;
    } | null;
    ...
    if (
      ...
      Object.keys(subscription.event_filter).length > 0
    ) {
...
export async function logAutomationEvent(
  ...
  try {
    const result = await processEventImmediately(
      siteId,
      data as AutomationEventLog,
    );
    ...
    console.log(
      `[Automation] Event ${eventType} processed: ${result.workflowsTriggered} workflows triggered`,
    );
...
async function processEventImmediately(
  ...
  if (error || !subscriptions?.length) {
    console.log(
      `[Automation] No active subscriptions found for ${event.event_type}`,
    );
    return { workflowsTriggered: 0 };
  }
  ...
  for (const sub of subscriptions) {
    const subscription = sub as EventSubscription & {
      workflow: {
        id: string;
        name: string;
        is_active: boolean;
    ...
    if (
      ...
      if (!matchesFilter(event.payload, subscription.event_filter)) {
        console.log(
          `[Automation] Event doesn't match filter for workflow: ${workflow.name}`,
        );
        continue;
      }
    }
    ...
    // Queue workflow execution
    console.log(
      `[Automation] Triggering workflow: ${workflow.name} for event ${event.event_type}`,
    );
    ...
    if (
      ...
      switch (op) {
        ...
        case "$lte":
          ...
          break;
        case "$contains":
          if (typeof actual !== "string" || !actual.includes(String(opValue)))
            return false;
          break;
        case "$in":
...
export async function processScheduledJobs(): Promise<{
  ...
  for (const job of jobs) {
    try {
      const workflow = job.workflow as {
        id: string;
        is_active: boolean;
        site_id: string;
      } | null;
 ...
 *
 * Simplified implementation that handles common cron patterns.
...
function calculateNextRun(cronExpression: string): string {
  ...
  // Try to parse simple patterns (minute hour * * *)
...
export async function processIncomingWebhook(
  ...
  const workflow = endpoint.workflow as {
    id: string;
    is_active: boolean;
  } | null;
  if (!workflow?.is_active) {
