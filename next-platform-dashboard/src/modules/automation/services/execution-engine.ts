...
export async function executeWorkflow(executionId: string): Promise<void> {
  ...
  try {
    ...
    while (currentStepIndex < steps.length) {
      ...
      // Execute step
      const result = await executeStepWithRetry(executionId, step, context);
...
/**
 * Execute a step with automatic retries based on step configuration.
 * Honors max_retries, retry_delay_seconds, and on_error settings.
 */
async function executeStepWithRetry(
  ...
  if (lastResult.status !== "failed" || maxRetries <= 0) {
    return lastResult;
  }
  ...
  for (let attempt = 2; attempt <= maxRetries + 1; attempt++) {
    ...
    lastResult = await executeStep(executionId, step, context, attempt);
...
async function executeStep(
  ...
  try {
    // Resolve input data
    const inputData = resolveVariables(
      step.input_mapping || {},
      context,
    ...
    switch (step.step_type) {
      ...
      case "stop":
        result = {
          status: "completed",
          output: { stopped: true, reason: step.action_config?.reason },
        };
        break;
...
async function executeActionStep(
  ...
  const config = resolveVariables(step.action_config || {}, context) as Record<
    string,
    unknown
  >;

  return executeAction(actionType, { ...inputData, ...config }, context);
}
...
async function evaluateCondition(
  ...
  const passed =
    operator === "and" ? results.every((r) => r) : results.some((r) => r);
 ...
 */
export async function resumePausedExecutions(): Promise<{
  resumed: number;
  errors: string[];
}> {
  const supabase = createAdminClient() as AutomationDB;
