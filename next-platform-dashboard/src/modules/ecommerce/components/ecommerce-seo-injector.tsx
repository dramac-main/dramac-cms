    ...
    if (settings?.quotation_mode_enabled && settings?.quotation_hide_prices) {
      ...
      return (
        <>
          ...
          <JsonLdScript
            data={generateWebSiteSearchJsonLd(storeName, siteUrl)}
          />
        </>
      );
    }
...
async function ProductPageSeo({
  ...
  const supabase = createAdminClient() as ReturnType<
  ...
  if (categoryLinks && categoryLinks.length > 0) {
    const categoryIds = categoryLinks.map(
      (l: { category_id: string }) => l.category_id,
    );
...
async function ShopPageSeo({
  ...
  const { data: products } = await db
    .from(`${TABLE_PREFIX}_products`)
    .select(
      "id, name, slug, images, base_price, track_inventory, quantity, status",
    )
    .eq("site_id", siteId)
    .eq("status", "active")
  ...
  if (products && products.length > 0) {
    schemas.push(
      generateProductListJsonLd(products as Product[], {
        ...options,
...
async function CategoryPageSeo({
  ...
  if (products.length > 0) {
    schemas.push(
      generateProductListJsonLd(products, {
        ...options,
