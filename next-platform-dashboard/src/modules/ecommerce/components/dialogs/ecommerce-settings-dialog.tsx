...
export function EcommerceSettingsDialog({
  open,
  onOpenChange,
}: EcommerceSettingsDialogProps) {
  ...
  const [allowBackorders, setAllowBackorders] = useState(
    settings?.continue_selling_when_out_of_stock ?? false,
  );
  ...
  const [orderNotifications, setOrderNotifications] = useState(
    settings?.send_order_confirmation ?? true,
  );
  ...
  const [flutterwavePublicKey, setFlutterwavePublicKey] = useState(
    (settings?.flutterwave_config as { public_key?: string } | null)
      ?.public_key || "",
  );
  const [flutterwaveSecretKey, setFlutterwaveSecretKey] = useState("");
  const [flutterwaveEnvironment, setFlutterwaveEnvironment] = useState<
    "test" | "live"
  >("test");
  const [pesapalConsumerKey, setPesapalConsumerKey] = useState(
    (settings?.pesapal_config as { consumer_key?: string } | null)
      ?.consumer_key || "",
  );
  const [pesapalConsumerSecret, setPesapalConsumerSecret] = useState("");
  const [pesapalEnvironment, setPesapalEnvironment] = useState<"demo" | "live">(
    "demo",
  );
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Configure your store settings</DialogDescription>
        </DialogHeader>
          ...
          <Tabs defaultValue="store" className="w-full">
            ...
            <TabsContent value="payments" className="space-y-4 mt-4">
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="flutterwave">
                      🦋 Flutterwave — African cards, mobile money, bank
                      transfer
                    </SelectItem>
              ...
              {paymentProvider === "flutterwave" && (
                <div className="space-y-4 border rounded-lg p-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      value={flutterwaveEnvironment}
                      onValueChange={(v) =>
                        setFlutterwaveEnvironment(v as "test" | "live")
                      }
                    >
              ...
              {paymentProvider === "pesapal" && (
                <div className="space-y-4 border rounded-lg p-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      value={pesapalEnvironment}
                      onValueChange={(v) =>
                        setPesapalEnvironment(v as "demo" | "live")
                      }
                    >
              ...
              {paymentProvider === "dpo" && (
                <div className="space-y-3 border rounded-lg p-4">
                  ...
                  <div className="flex items-start gap-2 p-3 rounded bg-muted text-xs text-muted-foreground">
                    ...
                    Configure your DPO company token and service type in the
                    full settings. Contact DRAMAC support to complete DPO
                    integration.
                  </div>
                </div>
              )}
              ...
              {paymentProvider === "manual" && (
                <div className="space-y-3 border rounded-lg p-4">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Each method becomes a tap-to-pay button in chat. Customers
                    pick one and Chiko sends just that method&apos;s details
              ...
              {!paymentProvider && (
                <div className="flex items-start gap-2 p-3 rounded-lg bg-amber-50 dark:bg-amber-950/20 text-amber-700 dark:text-amber-400 text-sm">
                  ...
                  No payment gateway selected. Customers will not be able to
                  complete purchases.
                </div>
              )}
            </TabsContent>
            ...
            <TabsContent value="shipping" className="space-y-4 mt-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Free shipping for orders above this amount. Leave empty to
                  disable.
                </p>
              </div>
            </TabsContent>
