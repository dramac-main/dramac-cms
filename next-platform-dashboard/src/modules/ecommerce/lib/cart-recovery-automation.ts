...
export async function processAbandonedCarts(
  ...
  try {
    ...
    const thresholdDate = new Date(
      now.getTime() - ABANDONMENT_THRESHOLD_HOURS * 60 * 60 * 1000,
    );
    ...
    for (const cart of abandonedCarts as AbandonedCartRow[]) {
      try {
        ...
        if (cart.recovery_email_sent_at) {
          ...
          const delayHours =
            RECOVERY_EMAIL_DELAYS[emailCount] ||
            RECOVERY_EMAIL_DELAYS[RECOVERY_EMAIL_DELAYS.length - 1];
          const nextSendTime = new Date(
            lastSent.getTime() + delayHours * 60 * 60 * 1000,
          );
        ...
        if (emailResult.success) {
          ...
          result.errors.push(
            `Failed to send email for cart ${cart.id}: ${emailResult.error || "unknown"}`,
          );
        }
      } catch (cartError) {
        result.errors.push(
          `Error processing cart ${cart.id}: ${cartError instanceof Error ? cartError.message : "unknown"}`,
        );
      }
    }
  } catch (error) {
...
export async function runCartRecoveryAutomation(): Promise<CartRecoveryResult> {
  ...
  console.log(
    `[CartRecovery] Processed ${aggregatedResult.processed} carts, sent ${aggregatedResult.emails_sent} emails, expired ${aggregatedResult.carts_expired} carts`,
  );
