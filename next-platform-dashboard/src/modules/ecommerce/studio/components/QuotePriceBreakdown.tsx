...
export function QuotePriceBreakdown({
  ...
  const format =
    formatPrice || ((p: number) => defaultFormatPrice(p, currency));
  ...
  const calculatedTax = quote ? quoteTaxAmount : afterDiscount * taxRate;
  ...
  const LineItem = ({
    ...
    >
      <span
        className={cn("text-muted-foreground", isBold && "text-foreground")}
      >
        {label}
      </span>
      <span
  ...
  return (
    <div className={cn("space-y-2", className)}>
      ...
      {showDetails && calculatedTax > 0 && (
        <LineItem
          label={taxRate > 0 ? `Tax (${(taxRate * 100).toFixed(0)}%)` : "Tax"}
          value={calculatedTax}
        />
      )}
      ...
      {/* Separator */}
      {variant === "detailed" && <Separator className="my-2" />}
...
export function QuoteSavingsDisplay({
  ...
  return (
    <div
      className={cn(
        "rounded-lg bg-success/5 border border-success/20 p-4 text-center",
        className,
      )}
    >
      <p className="text-sm text-success">Your Savings</p>
