  ...
  useEffect(() => {
    ...
    async function fetchCategory() {
      ...
      try {
        ...
        if (showProductCount) {
          try {
            const productsResult = await getPublicProductsByCategory(
              effectiveSiteId,
              found.id,
              1,
              1,
    ...
    return (
      ...
      >
        ...
        <p className="text-sm text-muted-foreground mt-1">
          {error || "This category may have been removed."}
        </p>
      </div>
    );
  }
...
export const categoryHeroDefinition: Omit<ComponentDefinition, "render"> = {
  ...
  label: "Category Hero",
  description:
    "Hero banner for category pages with name, description, image, and product count",
  category: "ecommerce",
  icon: "Image",
  defaultProps: {
    showImage: true,
  ...
  fields: {
    showImage: {
      type: "toggle",
      label: "Show Image",
      description: "Display category image as background",
    },
    showDescription: {
      type: "toggle",
