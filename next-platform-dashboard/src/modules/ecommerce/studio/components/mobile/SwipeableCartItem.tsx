...
export function SwipeableCartItem({
  ...
  const [isRevealed, setIsRevealed] = useState<"none" | "left" | "right">(
    "none",
  );
  ...
  const handleDragEnd = useCallback(
    (_event: MouseEvent | TouchEvent | PointerEvent, info: PanInfo) => {
      ...
      if (
        offset.x < -DELETE_THRESHOLD ||
        (offset.x < -SWIPE_THRESHOLD && velocity.x < -500)
      ) {
  ...
  return (
    <div className={cn("relative overflow-hidden rounded-lg", className)}>
      ...
      <motion.div
        drag="x"
        dragConstraints={{
          left: -DELETE_THRESHOLD,
          right: onMoveToWishlist ? DELETE_THRESHOLD : 0,
        }}
        dragElastic={0.1}
        onDrag={handleDrag}
