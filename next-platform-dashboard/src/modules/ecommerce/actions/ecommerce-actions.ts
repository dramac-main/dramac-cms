...
export async function updateOrderStatus(
  ...
  if (!allowed.includes(status)) {
    throw new Error(
      `Cannot transition order from '${currentOrder.status}' to '${status}'`,
    );
  }
...
export async function initializeEcommerceForSite(
  siteId: string,
  agencyId: string,
): Promise<void> {
  const supabase = await getModuleClient();
...
export async function generateOrderNumber(siteId: string): Promise<string> {
  ...
  // Fallback: Generate simple sequential number in JS
  const { count } = await supabase
    .from(`${TABLE_PREFIX}_orders`)
    .select("id", { count: "exact" })
    .eq("site_id", siteId);
