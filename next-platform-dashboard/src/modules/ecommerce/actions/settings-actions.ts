...
export async function getAllSettings(
  ...
  const currencyFromTopLevel =
    data.currency || defaultCurrencySettings.default_currency;
  const mergedCurrency: CurrencySettings = {
    ...
    default_currency:
      data.currency_settings?.default_currency || currencyFromTopLevel,
  };
...
export async function updateSettings(
  ...
  if (existing) {
    ...
    // Insert new
    const { error } = await supabase.from(`${TABLE_PREFIX}_settings`).insert({
      site_id: siteId,
      agency_id: agencyId,
      [column]: data,
    });
...
export async function updateNotificationSettings(
  ...
  if (result.success) {
    // Invalidate the template resolver cache so next email uses the fresh templates
    const { invalidateTemplateCache } =
      await import("@/lib/email/template-resolver");
    invalidateTemplateCache(siteId);
    // Invalidate the channel preference cache so notification dispatch uses fresh settings
    const { invalidateChannelCache } =
      await import("@/lib/services/notification-channel-resolver");
    invalidateChannelCache(siteId);
  }
