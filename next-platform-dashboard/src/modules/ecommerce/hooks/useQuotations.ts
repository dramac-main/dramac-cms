...
export interface UseQuotationsResult {
  ...
  updateBuilderItem: (
    productId: string,
    updates: Partial<QuoteBuilderItem>,
  ) => void;
...
async function fetchQuotes(
  ...
  const result = await getQuotes(siteId, {
    status: status?.[0],
    customerId: userId,
  });
...
async function createQuoteAction(
  ...
  const {
    createQuote: createQuoteServerAction,
    addQuoteItem,
    recalculateQuoteTotals,
    notifyQuoteCreated,
  } = await import("../actions/quote-actions");
...
async function rejectQuoteAction(
  token: string,
  reason?: string,
): Promise<boolean> {
...
export function useQuotations(
  ...
  const updateBuilderItem = useCallback(
    (productId: string, updates: Partial<QuoteBuilderItem>) => {
      setBuilderItems((prev) =>
        prev.map((item) =>
  ...
  const submitQuoteRequest = useCallback(
    async (data: QuoteRequestData): Promise<Quote | null> => {
      setSubmitError(null);
      ...
      try {
        ...
        const quote =
          currentQuote?.id === quoteId
            ? currentQuote
            : await fetchQuote(siteId, quoteId);
        if (!quote?.access_token) return false;

        const success = await acceptQuoteAction(
          quote.access_token,
          name,
          email,
          signature,
