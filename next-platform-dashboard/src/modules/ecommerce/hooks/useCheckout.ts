...
const STEPS: CheckoutStep[] = ["information", "payment"];
...
function buildPaymentMethods(
  ...
  const paddleConfig = settings.paddle_config as
    | { enabled?: boolean }
    | undefined;
  ...
  const fwConfig = settings.flutterwave_config as
    | { enabled?: boolean }
    | undefined;
  if (fwConfig?.enabled) {
    methods.push({
  ...
  const pesapalConfig = settings.pesapal_config as
    | { enabled?: boolean }
    | undefined;
...
function buildShippingMethods(
  ...
  const shippingZones = settings.shipping_zones as
    | Array<{
        id: string;
        name: string;
        description?: string;
...
function validateAddress(address: Partial<Address>): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
...
export function useCheckout(): UseCheckoutResult {
  ...
  const availableShippingMethods = useMemo(
    () =>
      buildShippingMethods(
        storefrontSettings as Record<string, unknown> | null,
      ),
    [storefrontSettings],
  );
  ...
  // Auto-select single shipping/payment method so customers skip trivial choices
  useEffect(() => {
  ...
  }, [availableShippingMethods, state.shippingMethod]);

  useEffect(() => {
    if (availablePaymentMethods.length === 1 && !state.paymentMethod) {
      setState((prev) => ({
        ...prev,
    ...
    try {
      ...
      if (saved) {
        ...
        setState((prev) => ({
          ...
          shippingAddress: prev.shippingAddress.first_name
            ? prev.shippingAddress
            : { ...prev.shippingAddress, ...data.address },
        }));
      }
    } catch {
  ...
  const totals = useMemo((): CartTotals => {
    ...
    return {
      ...
      total:
        safeCartTotals.subtotal -
        safeCartTotals.discount +
        safeCartTotals.tax +
        shippingAmount,
  ...
  const validation = useMemo((): CheckoutValidation => {
    ...
    if (shippingValidation.errors.length > 0)
    ...
    const canProceedByStep: Record<CheckoutStep, boolean> = {
      information:
        isEmailValid && shippingValidation.valid && !!state.shippingMethod,
