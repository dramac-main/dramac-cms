...
export async function createAgent(data: {
  ...
  try {
    ...
    if (!uuidRegex.test(data.userId)) {
      return {
        agent: null,
        error: "Invalid User ID format. Please select a valid team member.",
      };
    }
    ...
    if (existingAgent) {
      if (existingAgent.is_active) {
        return {
          agent: null,
          error:
            "This user is already registered as a chat agent for this site.",
        };
      }
...
export async function updateAgent(
  ...
  try {
    ...
    if (data.maxConcurrentChats !== undefined)
      updates.max_concurrent_chats = data.maxConcurrentChats;
  ...
  try {
    ...
    if (agent.role === "admin") {
      return {
        success: false,
        error:
          "Cannot remove the admin agent. The site owner must always be a chat agent.",
      };
    }
    ...
    if ((count || 0) <= 1) {
      return {
        success: false,
        error:
          "Cannot remove the last agent. At least one agent must remain active.",
      };
    }
  ...
  try {
    ...
    for (const agent of agents || []) {
      ...
      const responseTimes = (conversations || [])
        .filter(
          (c: Record<string, unknown>) => c.first_response_time_seconds != null,
        )
        .map(
          (c: Record<string, unknown>) =>
            c.first_response_time_seconds as number,
        );
      ...
      performance.push({
        ...
        avgResponseTime:
          responseTimes.length > 0
            ? Math.round(
                responseTimes.reduce((s: number, t: number) => s + t, 0) /
        ...
        avgRating:
          ratings.length > 0
            ? Number(
                (
                  ratings.reduce((s: number, r: number) => s + r, 0) /
                  ratings.length
                ).toFixed(2),
              )
            : 0,
