...
export async function routeConversation(
  ...
  let query = supabase
    ...
    .select(
      "id, user_id, display_name, status, current_chat_count, max_concurrent_chats, department_id, skills, is_active, last_assignment_at",
    )
    .eq("site_id", siteId)
    .eq("is_active", true)
    .eq("status", "online");
  ...
  if (targetDepartmentId) {
    query = query.or(
      `department_id.eq.${targetDepartmentId},department_id.is.null`,
    );
  }
...
export async function checkMissedConversations(siteId: string): Promise<{
  ...
  if (missedCount > 0) {
    ...
    const { EVENT_REGISTRY } =
      await import("@/modules/automation/lib/event-types");
    for (const conv of stale!) {
      logAutomationEvent(
        siteId,
        EVENT_REGISTRY.live_chat.conversation.missed,
        {
        ...
        },
        {
          ...
          sourceEntityId: (conv as Record<string, unknown>).id as string,
        },
      ).catch((err) =>
        console.error("[ChatRouting] Automation event error:", err),
      );
    }
    ...
    for (const conv of stale!) {
      const result = await routeConversation(
        siteId,
        (conv as Record<string, unknown>).id as string,
      );
