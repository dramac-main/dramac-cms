...
export function BulkActionBar({
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      toast.success(
        `Deleted ${result.processed} contact${result.processed !== 1 ? "s" : ""}`,
      );
      onClearSelection();
      onActionsComplete();
    } catch (err) {
  ...
  const handleBulkStatus = async (status: ContactStatus) => {
    ...
    try {
      ...
      toast.success(
        `Updated ${result.processed} contact${result.processed !== 1 ? "s" : ""}`,
      );
      onActionsComplete();
    } catch (err) {
      toast.error((err as Error).message);
  ...
  const handleBulkTag = async (tagName: string) => {
    ...
    try {
      ...
      toast.success(
        `Tagged ${result.processed} contact${result.processed !== 1 ? "s" : ""}`,
      );
      onActionsComplete();
    } catch (err) {
      toast.error((err as Error).message);
      ...
      <div className="flex items-center gap-3 px-4 py-3 bg-card border shadow-xl rounded-xl">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent>
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem onClick={() => handleBulkLeadStatus("new")}>
              Lead: New
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => handleBulkLeadStatus("qualified")}>
              Lead: Qualified
            </DropdownMenuItem>
            <DropdownMenuItem
              onClick={() => handleBulkLeadStatus("unqualified")}
            >
              Lead: Unqualified
            </DropdownMenuItem>
          </DropdownMenuContent>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent>
            <DropdownMenuItem onClick={() => handleBulkTag("VIP")}>
              Add: VIP
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => handleBulkTag("Hot Lead")}>
              Add: Hot Lead
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => handleBulkTag("Follow Up")}>
              Add: Follow Up
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => handleBulkTag("Newsletter")}>
              Add: Newsletter
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
