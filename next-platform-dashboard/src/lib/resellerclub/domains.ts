...
export class DomainService {
  ...
  async checkMultipleAvailability(
    ...
    const normalizedDomains = domainNames
      .filter(Boolean)
    ...
    const response = await this.client.get<
      Record<string, { status?: string; classkey?: string; classKey?: string }>
    >("domains/available.json", params, domainCheckUrl);
    ...
    for (const domain of domainTlds) {
      ...
      const result =
        response[keyWithDot] ??
        response[domain.full] ??
        responseLower[keyWithDot.toLowerCase()] ??
        response[keyNoDot] ??
        responseLower[keyNoDot.toLowerCase()];
      ...
      if (statusVal === "available") {
        ...
        statusVal === "regthroughus" ||
        statusVal === "regthroughothers"
      ) {
        status = "unavailable";
      } else if (
        typeof classKeyVal === "string" &&
        classKeyVal.toLowerCase().includes("premium")
      ) {
        status = "premium";
      }
  ...
  async getCustomerPricing(
    customerId?: string,
    tlds?: string[],
  ): Promise<Record<string, DomainPrice>> {
    ...
    >("products/customer-price.json", params);
  ...
  private unwrapSlabPricing(
    ...
    if (
      firstSlab &&
      typeof firstSlab === "object" &&
      !Array.isArray(firstSlab)
    ) {
  ...
  async register(
    ...
    const response = await this.client.post<{
      entityid: string;
      invoiceid?: string;
    }>("domains/register.json", apiParams);
  ...
  private mapDomainDetails(data: Record<string, unknown>): DomainDetails {
    return {
      ...
      ) as DomainDetails["currentStatus"],
      ...
      privacyProtection:
        data.isprivacyprotected === "true" || data.isprivacyprotected === true,
      transferLock:
        data.istransferlocked === "true" || data.istransferlocked === true,
      registrantContactId: String(
        data.registrantcontactid || data["registrant-contact-id"] || "",
      ),
      adminContactId: String(
        data.admincontactid || data["admin-contact-id"] || "",
      ),
      techContactId: String(
        data.techcontactid || data["tech-contact-id"] || "",
      ),
      billingContactId: String(
        data.billingcontactid || data["billing-contact-id"] || "",
      ),
   ...
   */
  async resendTransferApprovalEmail(
    orderId: string,
  ): Promise<{ success: boolean }> {
  ...
  validateDomainName(domainName: string): { valid: boolean; error?: string } {
    ...
    if (sld.length < 2) {
      return {
        valid: false,
        error: "Domain name too short (minimum 2 characters)",
      };
    }
    if (sld.length > 63) {
      return {
        valid: false,
        error: "Domain name too long (maximum 63 characters)",
      };
    }
