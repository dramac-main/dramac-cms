...
function findBestRoute(context: string, validRoutes: string[]): string {
  ...
  const mappings: [string[], string[]][] = [
    ...
    [
      ["about", "story", "who we", "our team", "meet"],
      ["/about", "/team"],
    ],
    [
      ["work", "portfolio", "project", "case stud"],
      ["/portfolio", "/work", "/gallery"],
    ],
    [
      ["shop", "product", "buy", "store", "purchase"],
      ["/shop", "/products"],
    ],
...
function fixLinksInObject(
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    const isNavLink =
      navLinkKeys.some((k) => keyLower === k.toLowerCase()) ||
      ((keyLower.includes("link") || keyLower.includes("href")) &&
    ...
    if (isNavLink) {
      if (typeof value === "string") {
        result[key] = fixLink(
          value,
          String(obj.label || obj.text || obj.title || obj.ctaText || ""),
        );
      } else {
        result[key] = value;
      }
...
function convertComponentToStudio(
  ...
  if (!KNOWN_REGISTRY_TYPES.has(resolvedType)) {
    ...
    const suffixStripped = resolvedType.replace(
      /(Block|Section|Component|Widget|Grid|List|Cards?|Panel|Area|Zone|Group|Bar|Row|Display|Wrapper|Item)$/i,
      "",
    );
    if (KNOWN_REGISTRY_TYPES.has(suffixStripped)) {
      console.log(
        `[Converter] Fuzzy match: "${resolvedType}" → "${suffixStripped}"`,
      );
      ...
      if (semanticMap[normalizedKey]) {
        console.log(
          `[Converter] Semantic match: "${resolvedType}" → "${semanticMap[normalizedKey]}"`,
        );
        resolvedType = semanticMap[normalizedKey];
      } else {
        console.warn(
          `[Converter] ⚠️ Unknown component type: "${genComponent.type}" → "${resolvedType}" — passing through as-is`,
        );
      }
    }
  }
...
function transformPropsForStudio(
  ...
  if (type === "Hero") {
    ...
    return {
      ...
      description:
        props.description || props.subheadline || props.subtitle || "",
      primaryButtonText: ctaText,
      primaryButtonLink: fixLink(
        String(
          props.ctaLink || props.buttonLink || props.primaryButtonLink || "",
        ),
        ctaText,
      ),
      ...
      primaryButtonColor:
        props.primaryButtonColor || props.buttonColor || undefined,
      primaryButtonTextColor:
        props.primaryButtonTextColor || props.buttonTextColor || undefined,
      ...
      backgroundOverlay: hasBackgroundImage
        ? true
        : (props.backgroundOverlay ?? false),
      backgroundOverlayOpacity: hasBackgroundImage
        ? props.backgroundOverlayOpacity || 70
        : props.backgroundOverlayOpacity || 0,
      // Split variant image
  ...
  if (type === "Navbar") {
    const links =
      props.links || props.navItems || props.navLinks || props.navigation || [];
    ...
    return {
      ...
      links: Array.isArray(links)
        ? links.map((link: Record<string, unknown>) => ({
            ...
            href: fixLink(
              String(link.href || link.url || link.link || ""),
              String(link.label || link.text || ""),
            ),
      ...
      ctaText,
      ctaLink: fixLink(
        String(props.ctaLink || props.buttonLink || ""),
        ctaText,
      ),
      ...
      layout: (() => {
        const v = String(
          props.variant || props.layout || "standard",
        ).toLowerCase();
  ...
  if (type === "CTA") {
    ...
    const resolvedButtonColor =
      props.buttonColor || props.primaryButtonColor || undefined;

    // CRITICAL: Normalize button text color — same mapping
    const resolvedButtonTextColor =
      props.buttonTextColor || props.primaryButtonTextColor || undefined;
    ...
    const resolvedTextColor = props.textColor
      ? colorsMatch(String(props.textColor), bgColor)
        ? bgIsLight
          ? "#0f172a"
          : "#ffffff"
        : props.textColor
    ...
    const resolvedTitleColor = props.titleColor
      ? colorsMatch(String(props.titleColor), bgColor)
        ? bgIsLight
          ? "#0f172a"
          : "#ffffff"
        : props.titleColor
    ...
    if (
      secondaryText &&
      String(secondaryText).toLowerCase() === buttonText.toLowerCase()
    ) {
  ...
  if (type === "Footer") {
    const linkColumns =
      props.columns || props.sections || props.linkColumns || [];
    ...
    return {
      ...
      // Link columns
      columns: Array.isArray(linkColumns)
        ? linkColumns.map((col: Record<string, unknown>, i: number) => ({
            title: col.title || col.heading || `Column ${i + 1}`,
            links: Array.isArray(col.links)
              ? col.links.map((link: Record<string, unknown>) => {
                  const label = String(
      ...
      socialLinks: Array.isArray(socialLinks)
        ? socialLinks.map((social: Record<string, unknown>) => ({
      ...
      showContactInfo: (() => {
        ...
        const isPlaceholder = (v: unknown) => {
          ...
          return (
            !s ||
            s.includes("555") ||
            s.includes("hello@company") ||
            s.includes("123 main") ||
            s.includes("info@company") ||
            s.includes("your@email") ||
            s.includes("example.com") ||
            s.includes("xxx") ||
            s.includes("000-000") ||
            s.includes("main street") ||
            s.includes("city, country") ||
            s.includes("city, state") ||
            s.includes("any street") ||
            s.includes("@yourbusiness") ||
            s.includes("@business") ||
            s.includes("lorem") ||
            s.includes("(000)") ||
            s.includes("000 000") ||
            (/\d{3}[\s-]?\d{3}[\s-]?\d{3,4}/.test(s) === false &&
              s.length > 3 &&
              /\d/.test(s))
          );
        };
      ...
      contactEmail: (() => {
        ...
        return /hello@company|info@company|your@|example\.com|@yourbusiness|@business\./i.test(
          e,
        )
          ? ""
          : e;
      ...
      contactAddress: (() => {
        ...
        return /123 main|main street|any street|city,?\s*(country|state)|lorem/i.test(
          a,
        )
          ? ""
          : a;
  ...
  if (type === "LogoCloud") {
    const logos =
      props.logos || props.items || props.brands || props.partners || [];
    // Filter out logos with no image URL — prevents broken images in production
    const processedLogos = Array.isArray(logos)
      ? logos
          .map((logo: Record<string, unknown>, i: number) => ({
    ...
    if (
      ...
      return {
        ...
        features: logos.map((logo: Record<string, unknown>, i: number) => ({
          id: String(i + 1),
          title: String(
            logo.alt || logo.name || logo.text || `Partner ${i + 1}`,
          ),
          description: "",
          icon: "🛡️",
        })),
  ...
  if (type === "TrustBadges") {
    ...
    return {
      ...
      badges: Array.isArray(badges)
        ? badges.map((b: Record<string, unknown>, i: number) => {
            const text = String(
              b.text ||
                b.title ||
                b.label ||
  ...
  if (type === "Quote") {
    return {
      ...
      authorTitle:
        props.authorTitle || props.source || props.role || props.company || "",
...
export function convertOutputToStudioPages(
  ...
  const allSlugs = output.pages.map((p) =>
