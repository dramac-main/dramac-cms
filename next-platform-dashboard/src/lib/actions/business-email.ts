...
export async function createBusinessEmailOrder(formData: FormData): Promise<{
  ...
  try {
    ...
    const numberOfAccounts = parseInt(
      formData.get("numberOfAccounts") as string,
    );
    const months = parseInt(formData.get("months") as string);
    const clientId = formData.get("clientId") as string | null;
    const productKey =
      (formData.get("productKey") as string | null) || "eeliteus";
    ...
    if (isNaN(months) || ![1, 3, 6, 12].includes(months)) {
      return {
        success: false,
        error: "Invalid subscription period. Choose 1, 3, 6, or 12 months.",
      };
    }
    ...
    if (
      ...
      console.warn(
        `[BusinessEmail] Clearing invalid RC customer ID "${customerId}"`,
      );
      customerId = undefined;
    }
    ...
    if (!customerId) {
      ...
      try {
        const { ensureResellerClubCustomerForAgency } =
          await import("@/lib/actions/domains");
        customerId = await ensureResellerClubCustomerForAgency(
          profile.agency_id,
          user.email || "",
        );
      } catch {
    ...
    if (!customerId) {
      return {
        success: false,
        error:
          "Agency not configured for email services. Please search for a domain first to set up your account.",
      };
    }
    ...
    const { data: agencyPricing } = await (supabase as any)
      ...
      .select(
        "default_markup_type, default_markup_value, pricing_source, apply_platform_markup",
      )
      .eq("agency_id", profile.agency_id)
      .single();

    // Try to get customer pricing first (includes RC markups)
    const rawCustomerPricing =
    ...
    const wholesalePrice = calculateBasePrice(
      wholesalePricing,
      months,
      numberOfAccounts,
      productKey,
    ...
    console.log(
      `[BusinessEmail] Pricing for ${domainName}: ${numberOfAccounts} accounts × ${months}mo → base=$${basePrice.toFixed(2)}, wholesale=$${wholesalePrice.toFixed(2)}`,
    );
    ...
    if (!retailPrice || retailPrice <= 0) {
      return {
        success: false,
        error: `Unable to calculate price for ${months}-month plan. Please try a different period.`,
      };
    }
    ...
    return {
      success: false,
      error:
        error instanceof Error ? error.message : "Failed to create email order",
    };
  }
}
...
export async function renewBusinessEmailOrder(
  ...
  try {
    if (![1, 3, 6, 12].includes(months)) {
      return {
        success: false,
        error: "Invalid renewal period. Choose 1, 3, 6, or 12 months.",
      };
    }
...
export async function getBusinessEmailStats(): Promise<{
  ...
  if (!orders)
    return {
      success: true,
      data: { total: 0, active: 0, accounts: 0, expiringSoon: 0 },
    };
  ...
  const stats = {
    total: orders.length,
    active: orders.filter((o: { status: string }) => o.status === "Active")
      .length,
    accounts: orders.reduce(
      (sum: number, o: { used_accounts: number }) => sum + o.used_accounts,
      0,
    ),
    expiringSoon: orders.filter(
      (o: { expiry_date: string }) =>
        new Date(o.expiry_date) < thirtyDaysFromNow,
    ).length,
  };
...
export async function getBusinessEmailAccounts(emailOrderId: string): Promise<{
  ...
  try {
    ...
    return {
      success: false,
      error:
        error instanceof Error ? error.message : "Failed to get email accounts",
    };
  }
}
...
export async function getBusinessEmailPricing(): Promise<{
  ...
  try {
    ...
    // STRATEGY: Cache-first with background live refresh.
    //
    // The cache stores reseller-level prices (may not reflect customer-specific markup).
    // We serve cache immediately for a fast initial load, then trigger a live fetch
    // in the background. The live fetch updates the cache so subsequent calls within
    // the same React Query staleTime window still get acceptable data, and the next
    // full refetch picks up the exact customer prices.
    //
    // Why cache-first? The RC API call takes 1-5s through the Fixie proxy. Serving
    // cache first eliminates the skeleton loading that frustrates users. Any minor
    // pricing delta between cache and Paddle is corrected by the live server-side
    // pricing calculation in createBusinessEmailOrder() before the Paddle transaction
    // is created — so the checkout price is always accurate.
    if (customerId) {
      // Try cache first for instant response
      customerPricing =
        await pricingCacheService.getAllCachedEmailPlans("customer");
      ...
      if (customerPricing) {
        ...
        try {
          ...
          console.error(
            "[BusinessEmail] Live pricing fetch failed, no cache available:",
            error,
          );
          return {
            success: false,
            error: "Failed to load pricing from provider",
          };
        }
      }
    } else {
      ...
      if (!customerPricing) {
        console.log(
          "[BusinessEmail] No customerId and no cache — fetching reseller pricing",
        );
        try {
          customerPricing = await businessEmailApi.getResellerPricing();
        } catch (error) {
          console.error(
            "[BusinessEmail] Reseller pricing fetch failed:",
            error,
          );
          return {
            success: false,
    ...
    costPricing = costPricing
      ? flattenTitanMailPricing(costPricing)
      : costPricing;
    ...
    const availablePlans = Object.keys(customerPricing).filter(
      (k) =>
        (customerPricing?.[k] as { email_account_ranges?: unknown })
          ?.email_account_ranges,
    );
    console.log(
      `[BusinessEmail] Pricing loaded — available plans: ${availablePlans.join(", ")}`,
    );
...
export async function getBusinessEmailDnsRecords(orderId: string): Promise<{
  ...
  try {
    ...
    const records = await emailDnsService.getDnsRecords(
...
export async function verifyBusinessEmailDns(orderId: string): Promise<{
  ...
  try {
    ...
    const result = await emailDnsService.verifyDnsRecords(
 ...
 */
export async function getBusinessEmailOrderByDomainId(
  domainId: string,
): Promise<{
  success: boolean;
  data?: {
  ...
  try {
    ...
    return {
      success: false,
      error:
        error instanceof Error ? error.message : "Failed to get email order",
    };
  }
}
 ...
 */
function flattenTitanMailPricing(
  ...
  for (const parentKey of TITAN_KEYS) {
    ...
    if (plans && typeof plans === "object") {
      ...
      for (const [planId, planData] of Object.entries(plans)) {
        ...
        if (hasRangeKeys) {
          result[syntheticKey] = {
            email_account_ranges: pd as Record<
              string,
              { add?: Record<string, number>; renew?: Record<string, number> }
            >,
          };
          foundAnyPlan = true;
        }
    ...
    for (const [subKey, subValue] of Object.entries(pv)) {
      if (
        ...
        result[`${parentKey}_${subKey}`] =
          subValue as EmailPricingResponse[string];
...
function isFreeTrial(plan: EmailPricingResponse[string] | undefined): boolean {
  ...
  const ranges = (
    plan as {
      email_account_ranges?: Record<string, { add?: Record<string, number> }>;
    }
  ).email_account_ranges;
...
function calculateBasePrice(
  ...
  if (!productPricing) {
    ...
    if (titanMatch) {
      ...
      if (parentPricing?.email_account_ranges) {
        console.log(
          `[EmailPricing] ${productKey} not found — using parent key ${parentKey} (Shape C flat pricing)`,
        );
        ...
        console.warn(
          `[EmailPricing] No pricing found for ${productKey} or parent ${parentKey}. Keys available:`,
          Object.keys(pricing),
        );
        return 0;
      }
      ...
      if (!fallback) {
        console.warn(
          `[EmailPricing] No pricing found for product key: ${productKey}`,
        );
        return 0;
      }
      console.warn(
        `[EmailPricing] No pricing for ${productKey}, falling back to eeliteus`,
      );
  ...
  if (!ranges || typeof ranges !== "object") {
    console.warn(
      "[EmailPricing] No email_account_ranges in pricing response. Keys:",
  ...
  if (!slab) {
    console.warn(
      `[EmailPricing] No slab found for ${accounts} accounts. Available slabs:`,
      Object.keys(ranges),
    );
    return 0;
  }
  ...
  if (price == null || isNaN(Number(price))) {
    console.warn(
      `[EmailPricing] No price for ${months} months in slab ${slab}. Available:`,
...
function findAccountSlab(
  ...
  console.warn(
    `[EmailPricing] No exact slab match for ${accounts} accounts, falling back to: ${firstSlab}`,
  );
...
function resolveTitanPlanId(productKey: string): number | null {
  ...
  const titanSyntheticMatch = productKey.match(
    /^titanmail(?:global|india)_(\d+)$/,
  );
  ...
  if (productKey === "enterpriseemailus")
    return TITAN_PLAN_IDS.global.enterprise;
  if (productKey === "eelitein") return TITAN_PLAN_IDS.india.business;
  if (productKey === "enterpriseemailin")
    return TITAN_PLAN_IDS.india.enterprise;
