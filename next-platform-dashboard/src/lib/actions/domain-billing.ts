  ...
  try {
    ...
    try {
      ...
      const { pricingCacheService } =
        await import("@/lib/resellerclub/pricing-cache");
      ...
      if (isClientAvailable()) {
        ...
        if (sellingPriceData && costPriceData) {
          switch (params.operation) {
            case "register": {
              const sellingPrices = sellingPriceData.register as Record<
                number,
                number
              >;
              const costPrices = costPriceData.register as Record<
                number,
                number
              >;
              retailPrice =
                sellingPrices[params.years] ||
                (sellingPrices[1] || 0) * params.years;
              wholesalePrice =
                costPrices[params.years] || (costPrices[1] || 0) * params.years;
              break;
            }
            case "renew": {
              const sellingPrices = sellingPriceData.renew as Record<
                number,
                number
              >;
    ...
    if (!usedRealPricing) {
      ...
      switch (params.operation) {
        case "register": {
          wholesalePrice =
            fb.register[params.years] || fb.register[1] * params.years;
    ...
    if (tldConfig?.enabled) {
      ...
      if (markupType === "custom") {
        const customPrices: Record<number, number> | undefined =
    ...
    if (markupType !== "custom" || retailPrice === 0) {
      if (usedRealPricing) {
        ...
        const effectiveMarkup =
          markupType === "percentage" && markupValue === 0 ? 30 : markupValue;
        switch (markupType) {
          case "percentage":
          ...
          case "fixed":
            retailPrice =
              wholesalePrice + (markupValue || wholesalePrice * 0.3);
            break;
          default:
    ...
    if (params.clientId && config?.client_tiers) {
      ...
      const clientTier = tiers.find((tier) =>
        tier.client_ids?.includes(params.clientId!),
      );
    ...
    const result: PricingCalculation = {
      ...
      markup_percentage:
        wholesalePrice > 0
          ? Math.round(
              ((retailPrice - wholesalePrice) / wholesalePrice) * 10000,
            ) / 100
          : 0,
      ...
      total_profit:
        Math.round(
          (retailPrice - wholesalePrice + privacyRetail - privacyWholesale) *
            100,
        ) / 100,
    };
...
export async function getBillingRecords(
  ...
  if (!profile?.agency_id)
    return { success: false, error: "No agency", data: [] };
 ...
 */
export async function getUsageHistory(months: number = 12): Promise<{
  success: boolean;
  data: DomainUsageSummary[];
  error?: string;
}> {
...
export async function getRevenueAnalytics(
  ...
  const totals = (records as AnyRecord[]).reduce<Accumulator>(
    (acc, record) => {
