...
export async function initiateTransferIn(formData: FormData) {
  ...
  try {
    ...
    if (
      ...
      customerIdToUse?.trim() === ""
    ) {
      console.warn(
        `[Transfers] Clearing invalid RC customer ID "${customerIdToUse}"`,
      );
      customerIdToUse = undefined;
    }
    ...
    if (!customerIdToUse) {
      try {
        ...
        customerIdToUse = await ensureResellerClubCustomerForAgency(
          profile.agency_id,
          user.email || "",
        );
      } catch {
    ...
    if (customerIdToUse) {
      try {
        ...
        console.warn(
          "[Transfer] ResellerClub API call failed, proceeding with local record:",
          apiError,
        );
    ...
    const { data: transfer, error } = await (
      adminClient as unknown as AnyQueryBuilder
    )
...
export async function getTransfers(type?: "in" | "out"): Promise<{
  ...
  try {
    ...
    const { data, error } = await baseQuery.order("initiated_at", {
      ascending: false,
    });
...
export async function getAuthCode(domainId: string) {
  ...
  try {
    ...
    const domain = domainRaw as
      | (DomainRecord & { domain_name?: string })
      | null;
    ...
    if (domain.resellerclub_order_id) {
      try {
        const authCode = await transferService.getAuthCode(
...
export async function setTransferLock(domainId: string, locked: boolean) {
  ...
  try {
    ...
    if (domain.resellerclub_order_id) {
      try {
        await transferService.setTransferLock(
          domain.resellerclub_order_id,
          locked,
        );
      } catch (apiError) {
        console.warn(
          "[Transfer] ResellerClub transfer lock update failed:",
          apiError,
        );
      }
    }
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : "Failed to update transfer lock",
    };
  }
}
  ...
  try {
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : "Failed to initiate transfer out",
    };
  }
}
