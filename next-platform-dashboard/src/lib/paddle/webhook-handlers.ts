...
export async function handlePaddleEvent(
  ...
  switch (event.eventType) {
    ...
    case "transaction.updated":
      ...
      console.log(
        `[Paddle Webhook] Transaction updated: ${event.data.id} (status: ${event.data.status})`,
      );
      break;
...
async function handleSubscriptionUpdated(
  ...
  const planChanged =
    existing.plan_type !== newPlanInfo.planType ||
  ...
  if (planChanged) {
    ...
    // BIL-06: Log plan change to subscription_events
    const changeType =
      newPlanLevel > oldPlanLevel ? "plan_upgraded" : "plan_downgraded";
    await logSubscriptionEvent(existing.agency_id, changeType, {
      subscription_id: data.id,
      old_plan: existing.plan_type,
      old_billing_cycle: existing.billing_cycle,
      new_plan: newPlanInfo.planType,
      new_billing_cycle: newPlanInfo.billingCycle,
...
async function handleSubscriptionCanceled(
  ...
  if (error || !sub) {
    console.error(
      "[Paddle Webhook] Error updating canceled subscription:",
      error,
    );
    return;
  }
  ...
  // BIL-07: Log cancellation to subscription_events
  await logSubscriptionEvent(sub.agency_id, "subscription.canceled", {
    subscription_id: data.id,
    reason: data.cancellationReason || "user_requested",
    canceled_at: new Date().toISOString(),
  });
...
async function handleSubscriptionPaused(
  ...
  if (sub) {
    ...
    // BIL-07: Emit automation event
    await emitBillingEvent(sub.agency_id, "billing.subscription.paused", {
      subscription_id: data.id,
      agency_id: sub.agency_id,
    });

    // BIL-07: Log to subscription_events
    await logSubscriptionEvent(sub.agency_id, "subscription.paused", {
      subscription_id: data.id,
      paused_at: new Date().toISOString(),
    });
  }
...
async function handleSubscriptionResumed(
  ...
  if (sub) {
    ...
    // BIL-07: Emit automation event
    await emitBillingEvent(sub.agency_id, "billing.subscription.resumed", {
      subscription_id: data.id,
      agency_id: sub.agency_id,
    });

    // BIL-07: Log to subscription_events
    await logSubscriptionEvent(sub.agency_id, "subscription.resumed", {
      subscription_id: data.id,
...
async function handleSubscriptionActivated(
  ...
  if (sub) {
    ...
    // Convert active trial if exists (trial → paid)
    try {
      await trialService.convertTrial(sub.agency_id, data.id);
    } catch {
      // No active trial to convert — that's fine
    }

    // Log event
    await logSubscriptionEvent(sub.agency_id, "subscription.activated", {
      subscription_id: data.id,
    });
  }
...
async function handleSubscriptionTrialing(
  ...
  const agencyId = data.customData?.agency_id;
  if (!agencyId) {
    console.error("[Paddle Webhook] No agency_id in trialing subscription");
    return;
  }
  ...
  // Log event
  await logSubscriptionEvent(agencyId, "subscription.trialing", {
    subscription_id: data.id,
    trial_end: data.trialPeriod?.endsAt,
  });

  console.log(`[Paddle Webhook] Subscription trialing: ${data.id}`);
}
...
async function handleTransactionCompleted(
  ...
  if (!sub) {
    console.error(
      "[Paddle Webhook] No subscription found for transaction:",
      data.subscriptionId,
    );
    return;
  }
  ...
  await supabase.from("paddle_transactions").upsert(
    {
      agency_id: sub.agency_id,
...
async function handleTransactionBilled(
  event: PaddleWebhookEvent,
): Promise<void> {
  const supabase = createAdminClient();
  const data = event.data;
  ...
  if (!sub) {
    console.log(
      "[Paddle Webhook] No subscription found for billed transaction",
    );
    return;
  }
...
function determinePlanFromPrice(priceId: string | undefined): {
  ...
  for (const [key, config] of Object.entries(PLAN_CONFIGS)) {
    if (config.priceId === priceId) {
      ...
      return {
        planType: planType as "starter" | "growth" | "agency",
  ...
  const planType = lowerPriceId.includes("agency")
    ? "agency"
    : lowerPriceId.includes("growth")
      ? "growth"
      : "starter";

  const billingCycle = lowerPriceId.includes("year") ? "yearly" : "monthly";
...
/**
 * Log a subscription event to the subscription_events table
 */
async function logSubscriptionEvent(
  ...
  try {
    ...
    console.error("[Paddle Webhook] Error logging subscription event:", err);
  }
}
 ...
 */
async function handleDomainEmailPurchaseCompleted(
  event: PaddleWebhookEvent,
): Promise<void> {
  const data = event.data;
  ...
  console.log(
    `[Paddle Webhook] Domain/Email purchase completed: ${transactionId}`,
  );

  try {
    // Import provisioning dynamically to avoid circular deps
    const { getPendingPurchaseByTransaction, updatePendingPurchaseStatus } =
      await import("@/lib/paddle/transactions");
    const {
    ...
    if (!purchase) {
      console.error(
        "[Paddle Webhook] No pending purchase found for transaction:",
        transactionId,
      );
      return;
    }
    ...
    if (purchase.status === "completed") {
      console.log(
        "[Paddle Webhook] Purchase already completed (idempotent):",
        purchase.id,
      );
      return;
    }
    ...
    if (purchase.status === "provisioning") {
      console.log(
        "[Paddle Webhook] Purchase already provisioning:",
        purchase.id,
      );
      return;
    }
    ...
    if (result.success) {
      ...
      try {
        ...
        const domainName =
          (purchase.purchase_data as any)?.domain_name || "unknown";
        ...
        if (purchaseType === "email_order") {
          ...
          const action = purchaseType.includes("register")
            ? "registered"
            : purchaseType.includes("renew")
              ? "renewed"
              : "transferred";
          await createNotification({
            userId,
        ...
        console.warn(
          "[Paddle Webhook] Failed to send success notification:",
          notifyErr,
        );
      }
    } else {
        ...
        if (refundResult.success) {
          ...
          try {
            ...
            await createNotification({
              ...
              metadata: {
                transactionId,
                adjustmentId: refundResult.adjustmentId,
              },
            });
          } catch (notifyErr) {
            console.warn(
              "[Paddle Webhook] Failed to send refund notification:",
              notifyErr,
            );
          }
        } else {
          ...
          try {
            await updatePendingPurchaseStatus(
              purchase.id as string,
              "refund_failed",
              {
    ...
    console.error(
      "[Paddle Webhook] Error handling domain/email purchase:",
      error,
    );
  }
}
...
async function emitBillingEvent(
  ...
  try {
    ...
    await (supabase as any).from("automation_events_log").insert({
      site_id: site.id,
      event_type: eventType,
