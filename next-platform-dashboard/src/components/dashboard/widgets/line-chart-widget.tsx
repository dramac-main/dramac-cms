...
export function LineChartWidget({
  ...
  const gradientIds = React.useMemo(
    () => dataKeys.map((_, i) => `gradient-${i}-${Math.random().toString(36).substr(2, 9)}`),
    [dataKeys]
  );
          ...
          >
            ...
            {dataKeys.map((key, index) => (
              <Line
                ...
                fill={gradients ? `url(#${gradientIds[index]})` : "transparent"}
              />
            ))}
          </LineChart>
        </ResponsiveContainer>
...
// Simplified line chart for cards
export function MiniLineChart({
  ...
  const gradientId = React.useMemo(
    () => `mini-gradient-${Math.random().toString(36).substr(2, 9)}`,
    []
  );
