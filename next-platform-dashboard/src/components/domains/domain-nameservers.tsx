...
const DEFAULT_NAMESERVERS = ["ns1.cloudflare.com", "ns2.cloudflare.com"];

const PLATFORM_NAMESERVERS = ["ns1.vercel-dns.com", "ns2.vercel-dns.com"];
...
export function DomainNameservers({
  ...
  const resetToDefaults = async (type: "cloudflare" | "platform") => {
    const defaultNs =
      type === "cloudflare" ? DEFAULT_NAMESERVERS : PLATFORM_NAMESERVERS;
    ...
    try {
      ...
      toast.success(
        `Reset to ${type === "cloudflare" ? "Cloudflare" : "Platform"} nameservers`,
      );
    } catch (error) {
        ...
        <TooltipProvider>
          <Tooltip>
            ...
            <TooltipContent side="left" className="max-w-xs">
              <p className="text-xs">
                Nameservers control where DNS queries for your domain are
                handled. Changes may take up to 48 hours to propagate globally.
              </p>
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
