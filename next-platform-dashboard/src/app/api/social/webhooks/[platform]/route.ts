...
import {
  analyzeSentiment,
  determinePriority,
} from "@/modules/social-media/lib/sentiment-analysis";
...
async function processMetaFeedChange(
  supabase: any,
  pageId: string,
  value: any,
) {
  ...
  await (supabase as any).from("social_inbox_items").upsert(
    {
      ...
      priority,
      platform_created_at: value.created_time
...
async function processMetaMessage(supabase: any, pageId: string, value: any) {
  ...
  const platformItemId =
    value.mid || value.message_id || `meta-msg-${Date.now()}`;
  ...
  await (supabase as any).from("social_inbox_items").upsert(
    {
      ...
      sentiment_score: sentimentResult.score,
      priority,
      platform_created_at: value.timestamp
      ...
      await (supabase as any).from("social_inbox_items").upsert(
        {
          ...
          sentiment_score: sentimentResult.score,
          priority,
          platform_created_at: tweet.created_at
