...
export async function GET(request: Request) {
  ...
  for (const account of accounts) {
    try {
      ...
      const { syncAccountAnalytics } =
        await import("@/modules/social-media/lib/analytics-sync-service");
      ...
      if (result.success) {
        ...
        failed++;
        errors.push(
          `analytics/${account.platform}/${account.id}: ${result.error}`,
        );
      }
    } catch (err: any) {
      failed++;
      errors.push(
        `analytics/${account.platform}/${account.id}: ${err.message?.slice(0, 100)}`,
      );
    }
  }
  ...
  for (const account of accounts) {
    try {
      const { syncAccountInbox } =
        await import("@/modules/social-media/lib/inbox-sync-service");
      ...
      if (result.newItems >= 0 && result.errors.length === 0) {
        ...
        inboxFailed++;
        errors.push(
          `inbox/${account.platform}/${account.id}: ${result.errors[0]}`,
        );
      }
    } catch (err: any) {
      inboxFailed++;
      errors.push(
        `inbox/${account.platform}/${account.id}: ${err.message?.slice(0, 100)}`,
      );
    }
  }

  console.log(
    `[Sync Cron] Analytics: ${synced} synced, ${failed} failed | Inbox: ${inboxSynced} synced, ${inboxFailed} failed`,
  );
