 ...
 * 
 * POST /api/billing/paddle/subscription/reactivate
 * 
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!member || member.role !== 'owner') {
      return NextResponse.json(
        { error: 'Only agency owners can reactivate subscriptions' },
        { status: 403 }
      );
    }
    
    // Reactivate subscription (remove scheduled cancellation)
    await subscriptionService.reactivateSubscription(member.agency_id);
