...
async function getModuleAndVerifyAccessLocal(
  supabase: any,
  moduleId: string,
  userId: string,
) {
...
async function handleDomainGet(
  ...
  try {
    ...
    const access = await getModuleAndVerifyAccessLocal(
      supabase,
      moduleId,
      user.id,
    );
...
async function handleDomainPatch(
  request: NextRequest,
  moduleId: string,
  domainId: string,
) {
...
async function handleWebhookGet(
  _request: NextRequest,
  moduleId: string,
  webhookId: string,
) {
  ...
  try {
    ...
    if (moduleError || !module) {
      const response = NextResponse.json(
        { error: "invalid_request", error_description: "Module not found" },
        { status: 404 },
      );
    ...
    switch (grant_type) {
      case "authorization_code":
        ...
        tokens = await oauthService.exchangeCode(
          code,
          client_id,
          client_secret,
          redirect_uri,
      ...
      case "refresh_token":
        ...
        tokens = await oauthService.refreshToken(
          refresh_token,
          client_id,
          client_secret,
        );
