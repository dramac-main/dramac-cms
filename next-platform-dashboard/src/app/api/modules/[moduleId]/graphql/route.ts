...
async function getModuleEntities(
  moduleId: string,
  installationId: string,
...
function parseGraphQLQuery(query: string): {
  ...
  const selectionSet = fieldMatches.filter(
    (f) => !["query", "mutation", operationName].includes(f),
  );
      ...
      if (variables.filter) {
        for (const [key, value] of Object.entries(variables.filter)) {
          if (typeof value === "object" && value !== null) {
            ...
            if (ops.eq !== undefined)
              queryBuilder = queryBuilder.eq(key, ops.eq);
            if (ops.neq !== undefined)
              queryBuilder = queryBuilder.neq(key, ops.neq);
            if (ops.gt !== undefined)
              queryBuilder = queryBuilder.gt(key, ops.gt);
            if (ops.gte !== undefined)
              queryBuilder = queryBuilder.gte(key, ops.gte);
            if (ops.lt !== undefined)
              queryBuilder = queryBuilder.lt(key, ops.lt);
            if (ops.lte !== undefined)
              queryBuilder = queryBuilder.lte(key, ops.lte);
            if (ops.like !== undefined)
              queryBuilder = queryBuilder.like(key, ops.like);
            if (ops.ilike !== undefined)
              queryBuilder = queryBuilder.ilike(key, ops.ilike);
            if (ops.in !== undefined)
              queryBuilder = queryBuilder.in(key, ops.in);
          } else {
...
export async function POST(
  ...
  try {
    ...
    if (parsed.operationType === "mutation") {
      if (
        !auth.consumer!.scopes.includes("write") &&
        !auth.consumer!.scopes.includes("*")
      ) {
        return NextResponse.json(
      ...
      if (
        !auth.consumer!.scopes.includes("read") &&
        !auth.consumer!.scopes.includes("*")
      ) {
        return NextResponse.json(
    ...
    await consumerService
      .logRequest(
        ...
        {
          method: "POST",
          path: `/graphql${operationName ? `?op=${operationName}` : ""}`,
        },
