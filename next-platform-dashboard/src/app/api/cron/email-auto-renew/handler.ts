...
export async function GET(request: Request) {
  ...
  if (!isConfigured()) {
    return NextResponse.json({
      skipped: true,
      reason: "ResellerClub not configured",
    });
  }
  ...
  const { data: candidates, error } = await admin
    .from("email_orders")
    .select(
      "id, agency_id, domain_name, expiry_date, resellerclub_order_id, auto_renew, auto_renew_months, product_key, number_of_accounts",
    )
    .eq("auto_renew", true)
    .eq("status", "Active")
  ...
  for (const order of candidates || []) {
    ...
    try {
      ...
      results.renewed++;
      console.log(
        `[EmailAutoRenew] Renewed ${order.domain_name} for ${months} months`,
      );
    } catch (err) {
      ...
      console.error(
        `[EmailAutoRenew] Failed to renew ${order.domain_name}:`,
        msg,
      );
    }
  }
