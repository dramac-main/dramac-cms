...
export function DomainSearchClient() {
  ...
  const handleAddToCart = (domain: DomainSearchResult) => {
    ...
    const wholesaleBase =
      Number(domain.prices.register[1] ?? domain.prices.register["1" as any]) ||
      1;
    const retailBase =
      Number(
        domain.retailPrices.register[1] ??
          domain.retailPrices.register["1" as any],
      ) || wholesaleBase;
    ...
    const item: DomainCartItem = {
      ...
      wholesalePrice:
        Number(
          domain.prices.register[1] ?? domain.prices.register["1" as any],
        ) || 0,
      retailPrice:
        Number(
          domain.retailPrices.register[1] ??
