  ...
  if (agencyId) {
    try {
      const { data: agency } = await (supabase as any)
        .from("agencies")
        .select(
          "id, name, custom_branding, white_label_enabled, default_currency, default_locale, default_timezone, date_format, tax_rate, tax_inclusive, weight_unit, dimension_unit",
        )
        .eq("id", agencyId)
        .single();
      if (agency) {
        regionalPreferences = {
          ...
          taxRate:
            agency.tax_rate != null ? Number(agency.tax_rate) : undefined,
        ...
        initialBranding = {
          ...
          agency_id: agency.id,
          agency_display_name:
            (cb.agency_display_name as string) ||
            (cb.display_name as string) ||
            agency.name ||
          ...
          primary_color:
            (cb.primary_color as string) || DEFAULT_BRANDING.primary_color,
          primary_foreground:
            (cb.primary_foreground as string) ||
            DEFAULT_BRANDING.primary_foreground,
          accent_color:
            (cb.accent_color as string) ||
            (cb.secondary_color as string) ||
            DEFAULT_BRANDING.accent_color,
          accent_foreground:
            (cb.accent_foreground as string) ||
            DEFAULT_BRANDING.accent_foreground,
          ...
          email_social_links:
            (cb.email_social_links as Record<string, string>) || {},
          portal_welcome_title: (cb.portal_welcome_title as string) || null,
          portal_welcome_subtitle:
            (cb.portal_welcome_subtitle as string) || null,
          portal_login_background_url:
            (cb.portal_login_background_url as string) || null,
          ...
          white_label_level:
            (cb.white_label_level as "basic" | "full" | "custom") ||
            (agency.white_label_enabled ? "full" : "basic"),
