...
for (const file of files) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/^(export\s+)?(default\s+)?function\s+[A-Z]/.test(line)) {
      ...
      if (hooksAfterReturn.length > 0) {
        ...
        for (const h of hooksAfterReturn) {
          console.log('  Hook after return - Line ' + h.line + ': ' + h.hook + ' - ' + h.text.substring(0, 100));
        }
      }
      inFunction = true;
      braceDepth = 0;
    ...
    if (inFunction) {
      ...
      if (braceDepth === 1 && /^\s*(if\s*\(|return\s)/.test(lines[i]) && /return\s/.test(lines[i])) {
        if (firstReturnLine === -1) firstReturnLine = i;
      }
      
      // Hooks after first return
      if (firstReturnLine !== -1 && i > firstReturnLine + 1) {
        const hookMatch = lines[i].match(/\b(useState|useEffect|useCallback|useMemo|useRef|useRouter|usePathname|useSearchParams|useParams|useTransition|useContext|useReducer|useId|useLayoutEffect|useFormState|useFormStatus|useOptimistic)\s*\(/);
        if (hookMatch && braceDepth >= 1) {
          hooksAfterReturn.push({ line: i + 1, hook: hookMatch[1], text: lines[i].trim() });
        }
      }
...
console.log('Total client component files scanned: ' + files.filter(f => {
