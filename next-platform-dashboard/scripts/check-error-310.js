...
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('310') && (lines[i].includes('Error') || lines[i].includes('error') || lines[i].includes('throw'))) {
...
if (idx310 > -1) {
  ...
  console.log('\nContext around first 310:', c.substring(start, end).substring(0, 200));
}
