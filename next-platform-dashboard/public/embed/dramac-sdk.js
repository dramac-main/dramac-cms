  ...
  var DramacSDK = {
    ...
    init: function (options) {
      ...
      if (!options.moduleId || !options.siteId || !options.token) {
        this._log(
          "error",
          "Missing required options: moduleId, siteId, and token are required",
        );
        if (options.onError) {
          options.onError(
            new Error("Missing required options: moduleId, siteId, token"),
          );
        }
        return null;
      }
      ...
      var instanceId =
        "dramac_" + Date.now() + "_" + Math.random().toString(36).substr(2, 9);
      ...
      loader.style.cssText =
        "position: absolute; top: 0; left: 0; right: 0; bottom: 0; display: flex; align-items: center; justify-content: center; background: #f9fafb; border-radius: 8px; font-family: system-ui, sans-serif; font-size: 14px; color: #6b7280;";
      loader.innerHTML =
        '<div style="width: 24px; height: 24px; border: 2px solid #e5e7eb; border-top-color: #3b82f6; border-radius: 50%; animation: dramac-spin 0.8s linear infinite; margin-right: 12px;"></div><span>Loading module...</span>';
      ...
      return {
        ...
        isReady: function () {
          return self._instances[instanceId]
            ? self._instances[instanceId].ready
            : false;
        },
    ...
    updateSettings: function (instanceId, settings) {
      ...
      if (
        ...
        this._log(
          "warn",
          "Cannot update settings - module not ready or destroyed",
        );
      }
    },
    ...
    getInstances: function () {
      ...
      for (var id in this._instances) {
        if (
          this._instances.hasOwnProperty(id) &&
          !this._instances[id].destroyed
        ) {
          result[id] = {
